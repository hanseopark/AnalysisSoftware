...
void FinaliseSystematicErrorsConvCalo_pp7TeV(TString nameDataFileErrors    = "",
                                            TString energy                = "", 
                                            TString meson                 = "", 
    ...
    TString nameCutVariationSC7TeV[nMaxVar] = {"YieldExtraction", "dEdxE", "dEdxPi", "TPCCluster", "SinglePt",
    ...
    for (Int_t k = 0; k < nMaxVar; k++){
        color[k]        = GetColorSystematics( nameCutVariationSC7TeV[k], 2);
        markerStyle[k]  = GetMarkerStyleSystematics( nameCutVariationSC7TeV[k], 2);
    }
    ...
    TString sFilePi0EtaBinning = "~/data/work/pcgGit/AnalysisSoftware/7TeV_Systematics/CutStudies/7TeV/Eta_data_SystematicErrorCuts.root";
    ...
    for (Int_t i = 0; i < nCuts; i++){
        ...
        if (i == 0 || i == 15 || i == 17 || i == 18 || i == 20 || i == 21 || ((i == 14 || i == 20) && (meson.CompareTo("Pi0EtaBinning") == 0)) ){ // special treatment for Yield extraction error and calculated erros
        ...
        if (bsmooth[i]){
            ...
            if  (nameCutVariationSC[i].CompareTo("YieldExtraction") == 0){
                if ( meson.CompareTo("Pi0") == 0){
                    for (Int_t k = 0; k < nPtBins; k++){
                        Double_t error            = 1.75+20/pow(25,ptBins[k]);
                    ...
                    for (Int_t k = 0; k < nPtBins; k++){
                        Double_t error            = 5.+25./pow(2.,ptBins[k]);
            ...
            if (nameCutVariationSC[i].CompareTo("ClusterMinEnergy")==0  ){
                for (Int_t k = 0; k < nPtBins; k++){
                    Double_t error          = 0.5+20/pow(10,ptBins[k]);
            ...
            if (nameCutVariationSC[i].CompareTo("ClusterTrackMatching")==0 ){
                if ( meson.CompareTo("Pi0") == 0){
                    for (Int_t k = 0; k < nPtBins; k++){
                        Double_t error          = 0.2+40/pow(100,ptBins[k]);
                        if(ptBins[k]>=4) error += 2E-3*(ptBins[k]-4)*(ptBins[k]-4)+0.2*(ptBins[k]-4);
                  ...
                  for (Int_t k = 0; k < nPtBins; k++){
                      Double_t error          = 0;
                      Double_t error2         = 0.2+40/pow(100,ptBins[k]);
                      if(ptBins[k]>=4) error2 += 2E-3*(ptBins[k]-4)*(ptBins[k]-4)+0.2*(ptBins[k]-4);
    ...
    DrawGammaCanvasSettings( canvasSummedErrMean, 0.08, 0.01, 0.015, 0.09);
        ...
        if (meson.CompareTo("Pi0EtaBinning") != 0){
            if (numberCutStudies>16 /*&& !(additionalNameOutput.CompareTo("") == 0)*/)
                legendSummedMeanNew->AddEntry(meanErrorsCorr[16],"SPD pileup","p");
        }
        DrawGammaSetMarkerTGraphErr(meanErrorsCorrSummedIncMat, 20, 1.,kBlack,kBlack);
        meanErrorsCorrSummedIncMat->Draw("p,csame");
