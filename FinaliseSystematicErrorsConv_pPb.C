...
void FinaliseSystematicErrorsConv_pPb(  const char* nameDataFileErrors      = "", 
	...
	TString nameCutVariationpPb5023GeV[14] = {"Yield extraction","Pile-up","dE/dx e-line","dE/dx #pi-line","TPC cluster","Single e^{#pm} p_{T}",  "#chi^{2} #gamma","q_{T}","#alpha meson", "#psi_{pair}","cos(#Theta_{point})","#eta_{#gamma, e^{#pm}}","BG method","MC smearing"};  
	TString nameCutVariation[14];
	TString nameCutVariationSC[14];
	
	TString nameCutVariationSCpPb5023GeV[14] = {"YieldExtraction","BGEstimate","dEdxE","dEdxPi", "Cluster", "SinglePt", "Chi2", "Qt", "Alpha", "PsiPair","CosPoint","Eta","BG","MCSmearing"};
	...
	gSystem->Exec("mkdir -p SystematicErrorsCalculated");
...
// 	}	
	...
	canvasSysErrMean->SaveAs(Form("SystematicErrorsCalculated/SysMean_%s_%s%s_%s.%s",meson.Data(), energy.Data(),additionalNameOutput.Data(),dateForOutput.Data(),suffix.Data()));
...
// 	}
	...
	canvasNewSysErrMean->SaveAs(Form("SystematicErrorsCalculated/SysMeanNewWithMean_%s_%s%s_%s.%s",meson.Data(), energy.Data(),additionalNameOutput.Data(),dateForOutput.Data(),suffix.Data()));
	
// 	delete canvasNewSysErrMean;

			const char *SysErrDatname = Form("SystematicErrorsCalculated/SystematicError_%s_%s%s_%s.dat",meson.Data(),energy.Data(),additionalNameOutput.Data(),dateForOutput.Data());
		fstream SysErrDat;
		...
		const char *SysErrDatnameMean = Form("SystematicErrorsCalculated/SystematicErrorAveraged_%s_%s%s_%s.dat",meson.Data(),energy.Data(),additionalNameOutput.Data(),dateForOutput.Data());
...
// 	}
	...
	canvasSummedErrMean->SaveAs(Form("SystematicErrorsCalculated/SysErrorSummedVisu_%s_%s%s_%s.%s",meson.Data(), energy.Data(),additionalNameOutput.Data(),dateForOutput.Data(),suffix.Data()));
	
	delete canvasSummedErrMean;

	
	
	const char *SysErrDatnameMeanPaper = Form("SystematicErrorsCalculated/SystematicErrorAveragedPaper_%s_%s%s_%s.dat",meson.Data(),energy.Data(),additionalNameOutput.Data(),dateForOutput.Data());
