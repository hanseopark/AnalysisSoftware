...
void FinaliseSystematicErrorsConv_pPb(  const char* nameDataFileErrors      = "",
    ...
    TString nameCutVariationpPb5023GeV[15]      = { "Yield extraction", "Pile-up", "dE/dx e-line", "dE/dx #pi-line", "TPC cluster", "Single e^{#pm} p_{T}", "#chi^{2} #gamma", "q_{T}","#alpha meson",
                                                    "#psi_{pair}", "cos(#Theta_{point})", "BG method", "MC smearing", "Yield extraction #pi^{0}", "#eta_{#gamma, e^{#pm}}"};
    TString nameCutVariation[15];
    TString nameCutVariationSC[15];

    TString nameCutVariationSCpPb5023GeV[15]    = { "YieldExtraction", "BGEstimate", "dEdxE", "dEdxPi", "Cluster",
                                                    "SinglePt", "Chi2", "Qt", "Alpha", "PsiPair",
                                                    "CosPoint", "BG", "Smearing", "YieldExtractionPi0", "Eta"};
