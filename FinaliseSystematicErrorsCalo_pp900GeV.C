...
void FinaliseSystematicErrorsCalo_pp900GeV(   const char* nameDataFileErrors  = "",
                                            TString energy                  = "", 
                                            TString meson                   = "", 
 ...
 // ***************************************************************************************************
    ...
    TString nameCutVariationSC900GeV[nMaxVar] = { "YieldExtraction", "OpeningAngle", "ClusterMinEnergy", "ClusterNCells", "ClusterNonLinearity",
    ...
    for (Int_t k =0; k<nMaxVar; k++ ){
        color[k]        = GetColorSystematics( nameCutVariationSC900GeV[k], 4 );
        markerStyle[k]  = GetMarkerStyleSystematics( nameCutVariationSC900GeV[k], 4 );
    }
    ...
    TString sFilePi0EtaBinning = "~/data/work/pcgGit/AnalysisSoftware/900GeV_CaloSystematics/CutStudies/900GeV/Eta_data_SystematicErrorCuts.root";
    ...
    for (Int_t i = 0;i < nCuts;i++){
        ...
        if (i == 0 || i == 7 || i == 8 || i == 11 || i == 13 || i == 14 || i == 15 || (i == 9 && meson.CompareTo("Pi0EtaBinning") == 0)){// special treatment for Yield extraction error and calculated erros
...
//                          if(ptBins[k]<=2.) error += -0.45+45/pow(10.,ptBins[k]);
                      error          = 1.5+25/pow(8,ptBins[k]);
                      if(ptBins[k]>=3.) error         += 0.6*(ptBins[k]-3) ;
            ...
            if (nameCutVariationSC[i].CompareTo("ClusterTrackMatchingCalo")==0 ){
                ...
                if ( meson.Contains("Pi0") && meson.CompareTo("Pi0EtaBinning") != 0){
                    ...
                    for (Int_t k = 0;k < nPtBins;k++){
                        Double_t error         = 2.5;
                        if(ptBins[k]>=5.) error += (0.2)*(ptBins[k]-5.);
                        errorsMean[i][k]        = error;
                        errorsMeanErr[i][k]     = 0.01*error;
                        errorsMeanCorr[i][k]    = error;
                        errorsMeanErrCorr[i][k] = 0.01*error;
