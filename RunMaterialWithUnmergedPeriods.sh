#! /bin/bash
   ...
   for inputFile in $inputFiles; do
      ...
      if [ -f $inputFile ]; then
         ...
         if [ $kFirstRun = 1 ]; then
			root -x -b -l -q TaskV1/BuildHistogramsForMaterialAdvV2.C\+\+\(\"$inputFile\"\,\"$cutnumber\"\,\"$cutnumberQA\"\,$kMC\,0\,\"$outputName\"\)
            kFirstRun=0;
         else    
            root -x -b -l -q TaskV1/BuildHistogramsForMaterialAdvV2.C\+\+\(\"$inputFile\"\,\"$cutnumber\"\,\"$cutnumberQA\"\,$kMC\,1\,\"$outputName\"\)
            kFirstRun=0;
         fi
      fi
   done;
