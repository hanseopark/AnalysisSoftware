...
)
{
    ...
    localFiles                                  = Form("../testSample%s.txt",dataType.Data());
    if(chunk != -1)
    ...
    // -----------------------------------------
        ...
        gInterpreter->ExecuteMacro(Form("$ALICE_ROOT/ANALYSIS/macros/AddTaskPIDResponse.C (%i, %i, %i, \"%s\")",intMCrunning,kFALSE,kTRUE,tenderPassData.Data())));
    #else
    ...
    // -----------------------------------------
    #if !defined (__CINT__) || defined (__CLING__)
        AddTask_V0Reader(runPeriod.Data(),kFALSE,kFALSE,kTRUE,collsys,aodConversionCutnumber.Data(), "00000008400100001500000000");
    #else
    ...
    if(runMode.Contains("P")){
        #if !defined (__CINT__) || defined (__CLING__)
            if(collsys==0){
                AddTask_GammaConvV1_pp(0, intMCrunning, "00000008400100001500000000", runPeriod.Data(),  2, 2, kFALSE, kFALSE, kFALSE, kFALSE,3, 0,"FMUW:fileNameMultWeights;FEPC:fileNamedEdxPostCalib", 0, "",kFALSE, "",0,kFALSE,kFALSE,kFALSE,1,0,0,"1020");
                AddTask_GammaConvV1_pp(0, intMCrunning, "00000008400100001500000000", runPeriod.Data(),  2, 2, kFALSE, kFALSE, kFALSE, kFALSE,3, 0,"FMUW:fileNameMultWeights;FEPC:fileNamedEdxPostCalib", 0, "",kFALSE, "",0,kFALSE,kFALSE,kFALSE,1,0,0,"400");
            }
            if(collsys==1){
