...
void PlotInterpolationPtBins(TGraphErrors** gPtvSqrts[],TGraphErrors** gPtvsEnergies[], TF1** fPowerlaw[], TGraphErrors* gRpPb,Int_t fColumnPlot, Int_t fRowPlot,TString namePlot){
    ...
    for(Int_t iPt = 0; iPt <gRpPb->GetN(); iPt++){
      ...
      Color_t typeColor[3] = {kBlue,kBlack,kBlack};
      
      DrawGammaSetMarkerTGraphErr(gPtvSqrts[0][iPt],21,1.5, kRed , kRed);
      DrawGammaSetMarkerTGraphErr(gPtvsEnergies[0][iPt],20,1.5, kBlack , kBlack);
      
      gPtvsEnergies[0][iPt]->GetXaxis()->SetTitle("#sqrt{s}");
      gPtvsEnergies[0][iPt]->GetXaxis()->SetTitleSize(0.065);
      gPtvsEnergies[0][iPt]->GetXaxis()->SetLabelSize(0.06);
      gPtvsEnergies[0][iPt]->GetYaxis()->SetTitle("invariant cross section");      
      gPtvsEnergies[0][iPt]->GetYaxis()->SetTitleSize(0.065);
      gPtvsEnergies[0][iPt]->GetYaxis()->SetLabelSize(0.06);
      gPtvsEnergies[0][iPt]->GetXaxis()->SetNdivisions(308,kTRUE);
      gPtvsEnergies[0][iPt]->GetYaxis()->SetNdivisions(304,kTRUE);
      gPtvsEnergies[0][iPt]->GetXaxis()->SetLabelOffset(0.015);
      gPtvsEnergies[0][iPt]->GetYaxis()->SetLabelOffset(0.01);
    
      gPtvsEnergies[0][iPt]->Draw("ap");
      gPtvSqrts[0][iPt]->Draw("same, p");
      for(Int_t i=0;i<3;i++){
      fPowerlaw[i][iPt]->SetLineColor(typeColor[i]);
      fPowerlaw[i][iPt]->SetLineWidth(2);
      fPowerlaw[i][iPt]->Draw("same");
      }
      
      TString Title = Form("#it{p}_{T} = %3.2f GeV/#it{c} ",gRpPb->GetX()[iPt]);
      
      if(Title.Length() > 0){
		gPtvsEnergies[0][iPt]->SetTitle("");
...
void PlotAlphavsPt(TGraphErrors* gAlpha[], TString method, TString thesisPlotLabel, TString namePlot){
    ...
    SetStyleTGraphErrorsForGraphs(gAlpha[0],pTLabel.Data(),"#alpha", 0.04,0.04, 0.04,0.04, 1.,1.2, 512, 512); 

    
    
    DrawGammaSetMarkerTGraphErr(gAlpha[0],21,1.5, kRed ,  kRed);
    DrawGammaSetMarkerTGraphErr(gAlpha[1],24,1.5, kGreen, kGreen);
    DrawGammaSetMarkerTGraphErr(gAlpha[2],26,1.5, kBlue,  kBlue);
    
    
    
    gAlpha[0]->GetYaxis()->CenterTitle();
   
    
    gAlpha[0]->Draw("ap");
    gAlpha[1]->Draw("same,p");
    gAlpha[2]->Draw("same,p");
