  ...
  for(Int_t i=0;i<13;i++){
    if(inputFile[i]){
      if(!inputFile[i]->IsZombie()){
        ...
        if(useLuciaPCM&&i==0){
          cout << "using LUCIA inputs for " <<  nameMeasGlobal[i] << endl;
...
// *******************************************************************************************************
  ...
  for (Int_t i = 0; i < totalNSets; i++){
    ...
    if(directoryPi0[i]){
      ...
      if(histoPi0Mass[i]&&i!=0) histoPi0Mass[i]->Scale(1000.);
      if(histoPi0FWHMMeV[i]&&i!=0) histoPi0FWHMMeV[i]->Scale(1000.);
      if(histoPi0TrueMass[i]&&i!=0) histoPi0TrueMass[i]->Scale(1000.);
      if(histoPi0TrueFWHMMeV[i]&&i!=0) histoPi0TrueFWHMMeV[i]->Scale(1000.);
    ...
    if(directoryEta[i]){
      ...
      if(histoEtaMass[i]&&i!=0) histoEtaMass[i]->Scale(1000.);
      if(histoEtaFWHMMeV[i]&&i!=0) histoEtaFWHMMeV[i]->Scale(1000.);
      if(histoEtaTrueMass[i]&&i!=0) histoEtaTrueMass[i]->Scale(1000.);
      if(histoEtaTrueFWHMMeV[i]&&i!=0) histoEtaTrueFWHMMeV[i]->Scale(1000.);
      ...
      if(histoEtaTrueEffPt[i] && histoEtaAcc[i]){
      ...
      if(i==2){
        histoEtaToPi0Stat[i]                        = (TH1D*)directoryEta[i]->Get("EtaToPi0Ratio");
        graphEtaToPi0Stat[i]                        = new TGraphAsymmErrors(histoEtaToPi0Stat[i]);
        graphEtaToPi0Sys[i]                         = (TGraphAsymmErrors*)directoryEta[i]->Get("EtaToPi0RatioSys");
      }
...
//     TGraphAsymmErrors* graph7TeVPi0SysPub           = (TGraphAsymmErrors*) file7TeVpub->Get("graphInvCrossSectionPi0Comb7TeVSysErr");
  ...
  TFile* fileEtaToPi                              = new TFile(fileNameEtaToPi0.Data());
      ...
      histoPythia8InvXSection->GetXaxis()->SetRangeUser(0.3,50);
      TH1F* histoPythia8InvXSectionEta            = (TH1F*) fileTheoryCompilation->Get("histoInvSecPythia8Monash2013LegoEta5TeV");
      histoPythia8InvXSectionEta->GetXaxis()->SetRangeUser(0.5,45);
      ...
      while(graphPythia8InvXSection->GetX()[graphPythia8InvXSection->GetN()-1] > 50.) graphPythia8InvXSection->RemovePoint(graphPythia8InvXSection->GetN()-1);
      ...
      while(graphPythia8InvXSectionEta->GetX()[graphPythia8InvXSectionEta->GetN()-1] > 45.) graphPythia8InvXSectionEta->RemovePoint(graphPythia8InvXSectionEta->GetN()-1);
      ...
      histoPythia8EtaToPi0->GetXaxis()->SetRangeUser(0.4,45);
      ...
      while(graphPythia8EtaToPi0->GetX()[graphPythia8EtaToPi0->GetN()-1] > 45.) graphPythia8EtaToPi0->RemovePoint(graphPythia8EtaToPi0->GetN()-1);

      // *******************************************************************************************************
      // NLO calc
      TGraphAsymmErrors* graphPi0DSS14            = (TGraphAsymmErrors*) fileTheoryCompilation->Get("graphNLOCalcDSS14InvSecPi05023GeV");
      while (graphPi0DSS14->GetX()[graphPi0DSS14->GetN()-1] > 50. ) graphPi0DSS14->RemovePoint(graphPi0DSS14->GetN()-1);
      ...
      while (graphNLOCalcEtaMuHalf->GetX()[graphNLOCalcEtaMuHalf->GetN()-1] > 45. )
          graphNLOCalcEtaMuHalf->RemovePoint(graphNLOCalcEtaMuHalf->GetN()-1);
      while (graphNLOCalcEtaMuOne->GetX()[graphNLOCalcEtaMuOne->GetN()-1] > 45. )
          graphNLOCalcEtaMuOne->RemovePoint(graphNLOCalcEtaMuOne->GetN()-1);
      while (graphNLOCalcEtaMuTwo->GetX()[graphNLOCalcEtaMuTwo->GetN()-1] > 45. )
      ...
      while (graphNLOEtaToPi0->GetX()[graphNLOEtaToPi0->GetN()-1] > 45. ) graphNLOEtaToPi0->RemovePoint(graphNLOEtaToPi0->GetN()-1);
...
cout << __LINE__ << endl;
  ...
  TLatex *labelRatioToFitBinShift   = new TLatex(0.72, 0.91, collisionSystem5TeV.Data());
  ...
  fitInvXSectionPi0           = FitObject("l","fitInvXSectionPi05TeV","Pi0",graphCombPi0InvXSectionTot,0.3,25.,paramGraph,"QNRMEX0+");
  ...
  TF1* fitTCMInvXSectionPi0        = FitObject("tcm","fitTCMInvXSectionPi05TeV","Pi0",graphCombPi0InvXSectionTot,0.4,40. ,paramTCMPi0New,"QNRMEX0+","", kFALSE);
  cout << WriteParameterToFile(fitTCMInvXSectionPi0)<< endl;

  Double_t paramPi0Power[3] = {1E11,0.5,6.5};
  TF1* fitPowInvXSectionPi0   = FitObject("powPure","fitPowInvXSectionPi05TeV","Pi0",graphCombPi0InvXSectionTot,3.5,40. ,paramPi0Power,"QNRMEX0+","", kFALSE);
  cout << WriteParameterToFile(fitPowInvXSectionPi0)<< endl;

  TF1* fitPowInvXSectionPi0Stat   = FitObject("powPure","fitPowInvXSectionPi05TeVStat","Pi0",graphCombPi0InvXSectionStat,3.5,40. ,paramPi0Power,"QNRMEX0+","", kFALSE);
  cout << WriteParameterToFile(fitPowInvXSectionPi0Stat)<< endl;

  Double_t paramPi0HageDorn[5] = {1E11,0.3,-0.1,0.5,5.95};
  TF1* fitOHagInvYieldPi0Tot   = FitObject("oHag","fitOHagInvYieldPi05TeV","Pi0",graphCombPi0InvXSectionTot,0.4,40. ,paramPi0HageDorn,"QNRMEX0+","", kFALSE);
  ...
  fitInvXSectionEta        = FitObject("l","fitInvXSectionEta5TeV","Eta",graphCombEtaInvXSectionTot,0.4,40.,paramGraphEta,"QNRMEX0+");
  ...
  TF1* fitTCMInvXSectionEta= FitObject("tcm","fitTCMInvXSectionEta5TeV","Eta",graphCombEtaInvXSectionTot,0.5,40.,paramTCMEta,"QNRMEX0+","", kFALSE);
  fitTCMInvXSectionEta     = FitObject("tcm","fitTCMInvXSectionEta5TeV","Eta",graphCombEtaInvXSectionTot,0.5,40.,paramTCMEta,"QNRMEX0+","", kFALSE);
  ...
  TF1* fitPowInvXSectionEta   = FitObject("powPure","fitPowInvXSectionEta5TeV","Eta",graphCombEtaInvXSectionTot,3.5,40. ,paramEtaPower,"QNRMEX0+","", kFALSE);
  cout << WriteParameterToFile(fitPowInvXSectionEta)<< endl;

  Double_t paramEtaHageDorn[5] = {1E11,0.3,-0.1,0.5,5.95};
  TF1* fitOHagInvYieldEtaTot   = FitObject("oHag","fitOHagInvYieldEta5TeV","Eta",graphCombEtaInvXSectionTot,0.4,40. ,paramEtaHageDorn,"QNRMEX0+","", kFALSE);
  cout << WriteParameterToFile(fitOHagInvYieldEtaTot)<< endl;

  TF1* fitPowInvXSectionEtaStat   = FitObject("powPure","fitPowInvXSectionEta5TeVStat","Eta",graphCombEtaInvXSectionStat,3.5,40. ,paramEtaPower,"QNRMEX0+","", kFALSE);
  ...
  Double_t columnsLegendOnlyPi0RatioAbs[3]= {0.15,pow(prodPtPi0,0.20), pow(prodPtPi0,0.325)};
...
Int_t rowcounter = 0;
  ...
  canvasRatioToCombFit->cd();
  histo2DEtaRatioToCombFit->GetYaxis()->SetRangeUser(0.2,3.45);
  ...
  SetStyleHistoTH2ForGraphs(histo2DAllEtaFWHM, "#it{p}_{T} (GeV/#it{c})", "Peak width (MeV/#it{c}^{2})", 0.85*textsizeLabelsWidth, textsizeLabelsWidth,
                            0.85*textsizeLabelsWidth, textsizeLabelsWidth, 0.8,0.28/(textsizeFacWidth*margin), 512, 505);
  histo2DAllEtaFWHM->GetYaxis()->SetRangeUser(-4.,69.5);
  ...
  TH2F * histo2DAllEtaMass            = new TH2F("histo2DAllEtaMass","histo2DAllEtaMass",20, minPtEta,maxPtEta, 1000., 485.1, 639.9);
  ...
  TH2F * histo2DAccEff;
  histo2DAccEff                = new TH2F("histo2DAccEff", "histo2DAccEff",1000, minPtPi0, maxPtPi0, 1000, 1.01e-4, 2 );
  ...
  legendRatioTheorypp_3Parted2->AddEntry((TObject*)0,"NLO, PDF:CTEQ6M5 - FF:DSS14", "");
  ...
  legendRatioTheorypp_3Parted3->AddEntry((TObject*)0,"NLO, PDF:CTEQ6M5 - FF:DSS14", "");
  ...
  TGraphAsymmErrors* graphCPionToPi0inputASys = (TGraphAsymmErrors*) graphChargedPionSysPP5TeV->Clone();
  TGraphAsymmErrors* graphCPionToPi0inputBSys = (TGraphAsymmErrors*) graphCombPi0InvXSectionSys->Clone();

  TGraphErrors* graphRatioBinByBinCPionToPi0 = CalculateRatioBetweenSpectraWithDifferentBinning(
                                                                                                      graphChargedPionStatPP5TeV->Clone(), graphCPionToPi0inputASys,
                                                                                                      graphCombPi0InvXSectionStat->Clone(), graphCPionToPi0inputBSys,
                                                                                                      kTRUE,  kTRUE,
  ...
  TGraphAsymmErrors* graphCPionToPi0inputASys_ForStat = (TGraphAsymmErrors*) graphChargedPionSysPP5TeV->Clone();
  TGraphAsymmErrors* graphCPionToPi0inputBSys_ForStat = (TGraphAsymmErrors*) graphCombPi0InvXSectionSys->Clone();
  ...
  TGraphErrors* graphRatioBinByBinCPionToPi0stat = CalculateRatioBetweenSpectraWithDifferentBinning(
                                                                                                      graphChargedPionStatPP5TeV->Clone(), graphCPionToPi0inputASys_ForStat,
                                                                                                      graphCombPi0InvXSectionStat->Clone(), graphCPionToPi0inputBSys_ForStat,
                                                                                                      kTRUE,  kTRUE,
                                                                                                      &graphRatioBinByBinCPionToPi0AStat, &graphRatioBinByBinCPionToPi0ASys,
                                                                                                      &graphRatioBinByBinCPionToPi0BStat, &graphRatioBinByBinCPionToPi0BSys )    ;
  ...
  padInvSectionSpec->SetLogx(1);
      ...
      TLatex *labelEnergyXSectionPaper= new TLatex(0.72, 0.91, collisionSystem5TeV.Data());
...
canvasEtatoPi0combo->SaveAs(Form("%s/Pi0_diffEnergy_ratio.%s",outputDir.Data(), suffix.Data()));
 ...
 TH2F * histoRatioEnergiesRa;
  ...
  histoRatioEnergiesRa->DrawCopy();
    ...
    // legendRatiosMC->Draw();
    DrawGammaLines(0.3, 25. , 1., 1.,0.5, kGray+2);
    histoRatioEnergiesRa->Draw("axis,same");
...
cout << "#######################################################################"<< endl;
  ...
  TDirectoryFile* fileDirectoryPi0 = (TDirectoryFile*)fCombResults.Get("Pi05TeV");
  fCombResults.cd("Pi05TeV");
      ...
      for (Int_t i = 0; i < 11; i++){
        if(directoryPi0[i]){
          if(graphPi0InvXSectionStat[i])graphPi0InvXSectionStat[i]          ->Write(Form("graphInvCrossSectionPi0%s5TeVStatErr",nameMeasGlobal[i].Data()));
          if(graphPi0InvXSectionSys[i])graphPi0InvXSectionSys[i]           ->Write(Form("graphInvCrossSectionPi0%s5TeVSysErr",nameMeasGlobal[i].Data()));
        }
      }
      ...
      if (bWCorrection.Contains("Y")){
          if(graphCombPi0InvXSectionTot_yShifted)graphCombPi0InvXSectionTot_yShifted->Write("graphInvCrossSectionPi0Comb5TeV_yShifted");
          if(graphCombPi0InvXSectionStat_yShifted)graphCombPi0InvXSectionStat_yShifted->Write("graphInvCrossSectionPi0Comb5TeVStatErr_yShifted");
          if(graphCombPi0InvXSectionSys_yShifted)graphCombPi0InvXSectionSys_yShifted->Write("graphInvCrossSectionPi0Comb5TeVSysErr_yShifted");

          for (Int_t i = 0; i < 11; i++){
            if(directoryPi0[i]){
              if(graphPi0InvXSectionStat_yShifted[i])graphPi0InvXSectionStat_yShifted[i]          ->Write(Form("graphInvCrossSectionPi0%s5TeVStatErr_yShifted",nameMeasGlobal[i].Data()));
              if(graphPi0InvXSectionSys_yShifted[i])graphPi0InvXSectionSys_yShifted[i]           ->Write(Form("graphInvCrossSectionPi0%s5TeVSysErr_yShifted",nameMeasGlobal[i].Data()));
            }
          }
      ...
      for (Int_t i = 0; i < 11; i++){
        if(directoryPi0[i]){
          ...
          if(histoPi0AccTimesEff[i])histoPi0AccTimesEff[i]          ->Write(Form("Pi0CorrectionFactor%s",nameMeasGlobal[i].Data()));
          if(histoPi0Mass[i])histoPi0Mass[i]                 ->Write(Form("Pi0MassData%s",nameMeasGlobal[i].Data()));
          if(histoPi0TrueMass[i])histoPi0TrueMass[i]             ->Write(Form("Pi0MassMC%s",nameMeasGlobal[i].Data()));
          if(histoPi0FWHMMeV[i])histoPi0FWHMMeV[i]              ->Write(Form("Pi0WidthData%s",nameMeasGlobal[i].Data()));
          if(histoPi0TrueFWHMMeV[i])histoPi0TrueFWHMMeV[i]          ->Write(Form("Pi0WidthMC%s",nameMeasGlobal[i].Data()));
        }
      }

  fCombResults.mkdir("Eta5TeV");
  TDirectoryFile* fileDirectoryEta = (TDirectoryFile*)fCombResults.Get("Eta5TeV");
  fCombResults.cd("Eta5TeV");
      ...
      for (Int_t i = 0; i < 11; i++){
        if(directoryEta[i]){
          if(graphEtaInvXSectionStat[i])graphEtaInvXSectionStat[i]          ->Write(Form("graphInvCrossSectionEta%s5TeVStatErr",nameMeasGlobal[i].Data()));
          if(graphEtaInvXSectionSys[i])graphEtaInvXSectionSys[i]           ->Write(Form("graphInvCrossSectionEta%s5TeVSysErr",nameMeasGlobal[i].Data()));
        }
      }
      ...
      if (bWCorrection.Contains("Y")){
          if(graphCombEtaInvXSectionTot_yShifted)graphCombEtaInvXSectionTot_yShifted->Write("graphInvCrossSectionEtaComb5TeV_yShifted");
          if(graphCombEtaInvXSectionStat_yShifted)graphCombEtaInvXSectionStat_yShifted->Write("graphInvCrossSectionEtaComb5TeVStatErr_yShifted");
          if(graphCombEtaInvXSectionSys_yShifted)graphCombEtaInvXSectionSys_yShifted->Write("graphInvCrossSectionEtaComb5TeVSysErr_yShifted");

          for (Int_t i = 0; i < 11; i++){
            if(directoryEta[i]){
              if(graphEtaInvXSectionStat_yShifted[i])graphEtaInvXSectionStat_yShifted[i]          ->Write(Form("graphInvCrossSectionEta%s5TeVStatErr_yShifted",nameMeasGlobal[i].Data()));
              if(graphEtaInvXSectionSys_yShifted[i])graphEtaInvXSectionSys_yShifted[i]           ->Write(Form("graphInvCrossSectionEta%s5TeVSysErr_yShifted",nameMeasGlobal[i].Data()));
            }
          }
      ...
      for (Int_t i = 0; i < 11; i++){
        if(directoryEta[i]){
          if(graphEtaToPi0Stat[i])graphEtaToPi0Stat[i]          ->Write(Form("graphEtaToPi0%s5TeVStatErr",nameMeasGlobal[i].Data()));
          if(graphEtaToPi0Sys[i])graphEtaToPi0Sys[i]           ->Write(Form("graphEtaToPi0%s5TeVSysErr",nameMeasGlobal[i].Data()));
        }
      }
      ...
      for (Int_t i = 0; i < 11; i++){
        if(directoryEta[i]){
          // Writing full correction factors
          if(histoEtaAccTimesEff[i])histoEtaAccTimesEff[i]          ->Write(Form("EtaCorrectionFactor%s",nameMeasGlobal[i].Data()));
          if(histoEtaMass[i])histoEtaMass[i]                 ->Write(Form("EtaMassData%s",nameMeasGlobal[i].Data()));
          if(histoEtaTrueMass[i])histoEtaTrueMass[i]             ->Write(Form("EtaMassMC%s",nameMeasGlobal[i].Data()));
          if(histoEtaFWHMMeV[i])histoEtaFWHMMeV[i]              ->Write(Form("EtaWidthData%s",nameMeasGlobal[i].Data()));
          if(histoEtaTrueFWHMMeV[i])histoEtaTrueFWHMMeV[i]          ->Write(Form("EtaWidthMC%s",nameMeasGlobal[i].Data()));
        }
      }
  fCombResults.Close();
...
// **********************************************************************************************************************
  ...
  TString nameOutputCommonFileFitsOnly    = Form("FitsPaperPP5TeV_%s.root", dateForOutput.Data());
