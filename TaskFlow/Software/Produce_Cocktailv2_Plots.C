...
void  Produce_Cocktailv2_Plots(
  ...
  TFile* fileData = new TFile("/home/mike/0_directphoton/13_DirectPhotonAnalysisCode/CocktailV2.root");
  
  TH1F*  hist1 = (TH1F*)fileData->Get(Form("v2gammaPi0_cen%i",centralityInt));
  hist1->SetLineColor(kBlack);
  hist1->SetMarkerColor(kBlack);
  hist1->SetMarkerStyle(20);
  
  TH1F*  hist2 = (TH1F*)fileData->Get(Form("v2gammaEta_cen%i",centralityInt));
  hist2->SetLineColor(kRed);
  hist2->SetMarkerColor(kRed);
  hist2->SetMarkerStyle(20);
  
  TH1F*  hist3 = (TH1F*)fileData->Get(Form("v2gammaK0s_cen%i",centralityInt));
  hist3->SetLineColor(kBlue);
  hist3->SetMarkerColor(kBlue);
  hist3->SetMarkerStyle(20);
  
  TH1F*  hist4 = (TH1F*)fileData->Get(Form("v2gammaOmega_cen%i",centralityInt));
  hist4->SetLineColor(kMagenta);
  hist4->SetMarkerColor(kMagenta);
  hist4->SetMarkerStyle(20);
  
  TH1F*  hist5 = (TH1F*)fileData->Get(Form("v2gammaCocktail_cen%i",centralityInt));
  ...
  if(CentralityLow.CompareTo("20")==0 || CentralityLow.CompareTo("40")==0){
    x1 = 0.18;
    y1 = 0.2;
    x2 = 0.4;
    y2 = 0.5;
  ...
  leg1->AddEntry(hist1,"#pi^{0}#rightarrow2#gamma","lp");
  leg1->AddEntry(hist2,"#eta#rightarrow2#gamma","lp");
  leg1->AddEntry(hist3,"K^{0}_{S}#rightarrow2#pi^{0}","lp");
  leg1->AddEntry(hist4,"#omega#rightarrow#gamma#pi^{0}","lp");
  ...
  histoDirectEmpty->GetXaxis()->SetRangeUser(0.9,14);
  ...
  c1->SaveAs(Form("Results/Cocktail/v2_gamma_input_%s_%s.eps",CentralityLow.Data(),CentralityHigh.Data()));
  c2->SaveAs(Form("Results/Cocktail/v2_gamma_total_%s_%s.eps",CentralityLow.Data(),CentralityHigh.Data()));
