...
TVectorD Rgamma(n_pt_bins);
TMatrixDSym cov_Rgamma(n_pt_bins);
TMatrixDSym cov_Rgamma_inv(n_pt_bins);
...
void chi2(Int_t &npar, Double_t *gin, Double_t &chi2, Double_t *par, Int_t iflag) {
    ...
    TVectorD Rgamma_model(n_pt_bins);
    for (Int_t i=0; i<n_pt_bins; ++i) {
	Double_t ptv = pt(i);
	Rgamma_model(i) = f(&ptv, par);
    }

    TVectorD diff = Rgamma - Rgamma_model;
    TVectorD tmp = cov_Rgamma_inv * diff;
...
// the main function
void test_fit_Rgamma() {

    // TString fname = "output_rgam_corr_coeff_025_2018_07_20/v2dir_pcm_phos_comb_20-40.root";
    TString fname = "output_test/v2dir_pcm_phos_comb_20-40.root";
    
    TFile* file = new TFile(fname.Data());

    TMatrixDSym cov_Rgamma_all_bins = *(TMatrixDSym*) file->Get("cov_Rgamma_comb_toterr");

    TGraphErrors g_Rgamma_all_bins = *(TGraphErrors*) file->Get("g_Rgamma_staterr");
    
    cov_Rgamma_all_bins.GetSub(pt_bin_low, pt_bin_up, pt_bin_low, pt_bin_up, cov_Rgamma);

    // invert covariance matrix
    cov_Rgamma_inv = cov_Rgamma;
    cov_Rgamma_inv.Invert();
    
    for (Int_t i=0; i<n_pt_bins; ++i) {
	pt(i) = g_Rgamma_all_bins.GetX()[i + pt_bin_low];
	Rgamma(i) = g_Rgamma_all_bins.GetY()[i + pt_bin_low];
    }

    //
    // now fit the Rgamma points
    //
    ...
    m.DefineParameter(0, "p0", 1.05, 0.01, 0, 0);
    m.DefineParameter(1, "p1", 0.05, 0.01, 0, 0);
    ...
    TH2F fr1("fr1", "fr1", 1, 0.8, 2.2, 1, 1., 1.15);
    fr1.SetXTitle("#it{p}_{T} (GeV/#it{c})");
    fr1.SetYTitle("#it{R}_{#gamma}");
    fr1.DrawCopy();

    g_Rgamma_all_bins.SetMarkerStyle(24);
    g_Rgamma_all_bins.DrawClone("p");

    TF1* ff = new TF1("ff", f, 0., 5., 3);
    // ff->SetParameters(p0, p1, p2);
    ff->SetParameters(p0, p1);
    ff->Draw("same");

    TLatex* la = new TLatex();
    la->SetTextSize(0.035);
    TString text = Form("chi2_per_ndf = %4.2f, p-value = %4.2f", chi2_per_ndf, p_value);
    la->DrawLatexNDC(0.2, 0.7, text);

    TLatex* la2 = new TLatex();
    la2->SetTextSize(0.025);
    la2->DrawLatexNDC(0.2, 0.8, fname);

    // do simple root fit in addition
    TF1* fs = new TF1("fs", "pol1", 0.9, 2.1);
    g_Rgamma_all_bins.Fit("fs", "0", "", 0.9, 2.1);
    fs->SetLineStyle(2);
    ...
    la->DrawLatexNDC(0.2, 0.65, text_simple);
    
    c1->SaveAs("test_fit_Rgamma.pdf");
}
