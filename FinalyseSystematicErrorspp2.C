...
void FinalyseSystematicErrorspp2(const char* nameDataFileErrors ="", TString energy="", TString meson = "", Int_t numberOfPtBins =1 ,Int_t numberCutStudies=1, Int_t offSetBeginning = 0, TString additionalName = "",TString additionalNameOutput = "",TString suffix = "eps",const char* sFilePi0EtaBinning=NULL){
    ...
    TString nameCutVariation8TeV[16]        = {"Yield extraction","Pile-up","dE/dx e-line","dE/dx #pi-line","TPC cluster","Single e^{#pm} p_{T}",  "#chi^{2} #gamma & #psi_{pair}","q_{T}","#alpha meson","BG method","Periods","SPD pileup","cos(#Theta_{point})","#psi_{pair}","#eta_{#gamma, e^{#pm}}","MC smearing"};  
    TString nameCutVariation[16];
    TString nameCutVariationSC[16];

    TString nameCutVariationSC8TeV[16]      = {"YieldExtraction_pp","BGEstimate_pp","dEdxE","dEdxPi", "TPCCluster", "SinglePt", "Chi2" , "Qt" , "Alpha" ,"BG" ,"7TeVPeriods" ,"SPD" ,"CosPoint", "PsiPair" ,"Eta" ,"MCSmearing"};
    ...
    for (Int_t i = 0; i < nCuts; i++){
        ...
        if (i == 0 && meson.CompareTo("EtaToPi0")){
            ...
            TGraphAsymmErrors* graphNegErrors1 = NULL;
            TGraphAsymmErrors* graphPosErrors1 = NULL;
            TGraphAsymmErrors* graphNegErrors2 = NULL;
            TGraphAsymmErrors* graphPosErrors2 = NULL;
        ...
        if (!energy.CompareTo("8TeV")){    
            ...
            if (nameCutVariationSC[i].CompareTo("dEdxE")==0){
                if (meson.CompareTo("Pi0")==0 ) {
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] < 1.5){
                            errorsMean[i][k]        = 0.5+pow(ptBins[k]-1.5,2)*1.3;
                            errorsMeanErr[i][k]     = 0.02;
                            errorsMeanCorr[i][k]    = 0.5+pow(ptBins[k]-1.5,2)*1.3;
                            errorsMeanErrCorr[i][k] = 0.02;
                        }
                        if (ptBins[k] > 1.5){
                            errorsMean[i][k]        = 0.5;
                    ...
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] < 2.0){
                            errorsMean[i][k]        = 0.5+pow(ptBins[k]-1.5,2)*2.3;
                            errorsMeanErr[i][k]     = 0.001;
                            errorsMeanCorr[i][k]    = 0.5+pow(ptBins[k]-1.5,2)*2.3;
                            errorsMeanErrCorr[i][k] = 0.001;
                        }
                        if (ptBins[k] > 2.0){
                            errorsMean[i][k]        = 0.5;
            ...
            if (nameCutVariationSC[i].CompareTo("dEdxPi")==0){
                if (meson.CompareTo("Pi0")==0 ) {
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.15+pow(ptBins[k],2)*0.012;
                            errorsMeanErr[i][k]     = 0.02;
                            errorsMeanCorr[i][k]    = 0.15+pow(ptBins[k],2)*0.012;
                            errorsMeanErrCorr[i][k] = 0.02;
                        }
                    }   
                }
                    ...
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.1+pow(ptBins[k],2)*0.01;
                            errorsMeanErr[i][k]     = 0.001;
                            errorsMeanCorr[i][k]    = 0.1+pow(ptBins[k],2)*0.01;
                            errorsMeanErrCorr[i][k] = 0.001;
                        }
                    }   
                }
            ...
            if (nameCutVariationSC[i].CompareTo("Qt")==0){
                if (meson.CompareTo("Pi0")==0 ) {
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.2+pow(ptBins[k],2)*0.028;
                            errorsMeanErr[i][k]     = 0.02;
                            errorsMeanCorr[i][k]    = 0.2+pow(ptBins[k],2)*0.028;
                            errorsMeanErrCorr[i][k] = 0.02;
                        }
                    }   
                }
                    ...
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.5+pow(ptBins[k],2)*0.035;
                            errorsMeanErr[i][k]     = 0.001;
                            errorsMeanCorr[i][k]    = 0.5+pow(ptBins[k],2)*0.035;
                            errorsMeanErrCorr[i][k] = 0.001;
                        }
                    }   
                }
            ...
            if (nameCutVariationSC[i].CompareTo("Chi2")==0){
                if (meson.CompareTo("Pi0")==0 ) {
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.75+pow(ptBins[k],2)*0.015;
                            errorsMeanErr[i][k]     = 0.02;
                            errorsMeanCorr[i][k]    = 0.75+pow(ptBins[k],2)*0.015;
                            errorsMeanErrCorr[i][k] = 0.02;
                        }
                    }   
                }
                    ...
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 1.2+pow(ptBins[k]-3,2)*0.15;
                            errorsMeanErr[i][k]     = 0.001;
                            errorsMeanCorr[i][k]    = 1.2+pow(ptBins[k]-3,2)*0.15;
                            errorsMeanErrCorr[i][k] = 0.001;
                        }
                    }   
                }
            ...
            if (nameCutVariationSC[i].CompareTo("Alpha")==0){
                if (meson.CompareTo("Pi0")==0 ) {
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.1+pow(ptBins[k],2)*0.035;
                            errorsMeanErr[i][k]     = 0.02;
                            errorsMeanCorr[i][k]    = 0.1+pow(ptBins[k],2)*0.035;
                            errorsMeanErrCorr[i][k] = 0.02;
                        }
                    }   
                }
                    ...
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.1+pow(ptBins[k],2)*0.045;
                            errorsMeanErr[i][k]     = 0.001;
                            errorsMeanCorr[i][k]    = 0.1+pow(ptBins[k],2)*0.045;
                            errorsMeanErrCorr[i][k] = 0.001;
                        }
                    }   
                }
            ...
            if (nameCutVariationSC[i].CompareTo("BG")==0){
                if (meson.CompareTo("Pi0")==0 ) {
                    for (Int_t k = 0; k < nPtBins; k++){
                        if (ptBins[k] > 0.0){
                            errorsMean[i][k]        = 0.1;
                            errorsMeanErr[i][k]     = 0.002;
                            errorsMeanCorr[i][k]    = 0.1;
                            errorsMeanErrCorr[i][k] = 0.002;
                        }
                    }   
                }
//                 else if (meson.CompareTo("Eta")==0 ) {
...
//                 }
    ...
    // 	if (meson.CompareTo("Pi0")==0 ){
    histo2DSysErrMean = new TH2D("histo2DSysErrMean", "histo2DSysErrMean", 20,0.,ptBins[nPtBins-1]+2,1000.,0.,25.);
    // 	} else {
    ...
    *		legendMeanNew= new TLegend(0.15,0.6,0.57,0.95);
    ...
    // 	if (meson.Contains("Pi0") ){	
    histo2DSummedErrMean = new TH2D("histo2DSummedErrMean", "histo2DSummedErrMean", 20,0.,ptBins[nPtBins-1]+2,1000.,0,25.);
    // 	} else {
