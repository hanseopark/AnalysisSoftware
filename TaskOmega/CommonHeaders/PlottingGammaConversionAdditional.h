...
void DrawAliceLogoPi0WithPHOSPerformance(Float_t startTextX, Float_t startTextY, Float_t startPi0TextX, Float_t differenceText,
						   Float_t startLogoX, Float_t startLogoY, Float_t widthLogo, 
...
void DrawAliceLogoPi0PerformanceExtract(Float_t startTextX, Float_t startTextY, Float_t startPi0TextX, Float_t differenceText,
   ...
   if(startPi0TextX){}
	...
	//if (pi0label==2) decayChainText = "#omega #rightarrow pi^{+}pi^{-}pi^{0}";
	...
	TLatex *alice = new TLatex(startTextX,(startTextY+(2*differenceText)),Form("%s",aliceText.Data())); // Bo: this was modified
	TLatex *energy = new TLatex(startTextX,(startTextY+differenceText),collisionSystem.Data()); // Bo: this was modified
	TLatex *process = new TLatex(startTextX, (startTextY-(2*differenceText)), decayChainText.Data());
	TLatex *events = 0x00;
		TLatex *latexDate = 0x00;
	if (nEvents != 0){ 
		events = new TLatex(startTextX,startTextY,Form("%s: %2.1e  MinBias events",eventsText.c_str(), nEvents)); // Bo: this was modified
...
void PrintLevyFitResults(Float_t startX, Float_t startY , Float_t spacingxVal, Float_t spacingxText, Float_t spacingy ,Size_t textsize, 
					Double_t* fitResults){
...
void DrawBinShiftingText(Float_t startX, Float_t startY , Size_t textsize, Double_t differenceText=0.021){  // kk
...
void DrawPrevPrelimPi0(Float_t startX, Float_t startY , Size_t textsize){  // kk
...
void DrawNormalizationErrorText(Float_t startX, Float_t startY , Float_t spacingx, Float_t spacingy ,Size_t textsize, TString energy){
...
void DrawIndividualTextSlicesR ( Double_t* yValues, Size_t textsize, TString option ){
...
void DrawAliceLogoAdv(Double_t x_left, Double_t y_down, TString type, TString energy, TString position, TCanvas *myCanvas, TString centrality){
...
void DrawCentrality(Double_t x_low, Double_t y_low, TString centrality){
...
void DrawSystem(Double_t x_low, Double_t y_low, TString energy, TString centrality){
...
TCanvas *GetAndSetCanvas(TString name, Double_t leftmargin = 0.11, Double_t bottommargin = 0.1,Double_t x = 1400 ,Double_t y = 1000){
...
TLegend *GetAndSetLegend(Double_t positionX, Double_t positionY, Double_t entries, Int_t Columns = 1, TString header =""){

	if(header.CompareTo("") != 0) entries++;
	Double_t positionYPlus = 0.05*(Double_t)entries;
...
void SetHistogramm(TH1 *hist, TString xLabel, TString yLabel, Double_t rangeYlow = -99., Double_t rangeYhigh = -99., Double_t xOffset = 1.0, Double_t yOffset = 1.15){
...
void SetGraph(TGraph *graph, TString xLabel, TString yLabel, Double_t rangeYlow = -99., Double_t rangeYhigh = -99., Double_t xOffset = 1.0, Double_t yOffset = 1.15){
...
//THE FOLLOWING FUNCTION COULD BE REALLY BADLY PROGRAMMED. IT IS A CHANGED COPY OF DrawAliceLogoPi0PerformanceExtract

void DrawAliceLogoOmegaPerformance(Float_t startTextX, Float_t startTextY, Float_t startPi0TextX, Float_t differenceText,
						   Float_t startLogoX, Float_t startLogoY, Float_t widthLogo, 
						   Float_t textSize, Float_t nEvents,
						   TString collisionSystem, Bool_t mcFile , Bool_t rawData, Bool_t pi0Label, Double_t xLengthCanvas, Double_t yLengthCanvas,TString date= "",Bool_t dalitz= kFALSE){
   ...
   if(startPi0TextX){}
	...
	string processText;
	string eventsText;
	if (pi0Label) {processText = "#omega";}
	else {processText = "#omega";}
	...
	decayChainText = "#omega #rightarrow #pi^{+}#pi^{-}#pi^{0}";
