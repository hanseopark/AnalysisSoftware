...
//#include "ProduceFinalResults.h" //This file have problems
...
void ElectronQAv1(TString outputData="",TString outputMC="", TString fCutSelection="8000011002093603007200000000_9047540023310262371_01031035009000",TString Suffix="eps",TString energy="pPb_5.023TeV", TString period="",Int_t mode = 9){
	...
	if( fElectronCutsData ) {
	   ...
	   hTPCEledEdxSigafter_data 					= (TH2F*)fElectronCutsData->FindObject(Form("Electron_dEdxSignal_after %s",fCutSelection.Data() )); //Change name of the id for the histogram, fElectronCutSelection.
	   
	   //hTPCdEdxafter->Print();
           //cout<<fElectronCutSelection<<"Electron_dEdx_after"<<fCutSelection<<endl; //Chech for the id of the cuts
	   
	   hTPCEledEdxafter_data 					= (TH2F*)fElectronCutsData->FindObject(Form("Electron_dEdx_after %s",fCutSelection.Data() )); //Same Here
	...
	if( fElectronCutsMC ) {
	  //Same here changing id from the cut, from fElectronCutSelection to fCutSelection
	   hTPCEledEdxSigafter_mc 					= (TH2F*)fElectronCutsMC->FindObject(Form("Electron_dEdxSignal_after %s",fCutSelection.Data() ));
