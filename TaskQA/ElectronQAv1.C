...
void ElectronQAv1(TString outputData="",TString outputMC="", TString fCutSelection="8000011002093603007200000000_9047540023310262371_01031035009000",TString Suffix="eps",TString energy="pPb_5.023TeV", TString period="",Int_t mode = 9){
	...
	if( fElectronCutsData ) {
	   ...
	   hTPCEledEdxSigafter_data 					= (TH2F*)fElectronCutsData->FindObject(Form("Electron_dEdxSignal_after %s",fCutSelection.Data() ));

	   //hTPCdEdxafter->Print(); Here we change the name fElectronCutSelection for fCutSelection
           cout<<fElectronCutSelection<<"Electron_dEdx_after"<<endl; //Here is the check

	   hTPCEledEdxafter_data 					= (TH2F*)fElectronCutsData->FindObject(Form("Electron_dEdx_after %s",fCutSelection.Data() ));
	...
	if( fQAFolderMC ) {
            cout<<"Check folder QA"<<endl;//Check for the folder QA
	...
	if( fConvCutsMC ) {
            cout<<fGammaCutSelectionMC<<"Sig-after"<<endl; //Here is a problem, never come inside here...
	...
	if( fElectronCutsMC ) {
            cout<<"Entro Electron cuts MC"<< fCutSelection <<endl;
            hTPCEledEdxSigafter_mc = (TH2F*)fElectronCutsMC->FindObject(Form("Electron_dEdxSignal_after %s",fCutSelection.Data() )); //Rename the cut for fElectronCutSelection to fCutSelection.
            hTPCEledEdxafter_mc = (TH2F*)fElectronCutsMC->FindObject(Form("Electron_dEdx_after %s",fCutSelection.Data() ));
            //Same here, rename fElectronCutSelection for fCutSelection.
...
/*Comment this Histogram with problems
