...
void BuildHistogramsForGammaQAAdvV3(
    ...
    if(addExtFolderToOutput){
        outputFolderName            = Form("GammaConvV1_QA_%s_%s", fCutSelection.Data(), specificCutSelection.Data());
    }else{
        outputFolderName            = Form("GammaConv_%s",  specificCutSelection.Data());
    }
    ...
    if(treeList)
        ...
        if(!PhotonQA)
            PhotonQA                = (TTree*)f->Get(Form("PhotonQA_%s",fCutSelection.Data()));
    ...
    TDirectory*  directoryConv      = (TDirectory*)fileMappingDetailedConv->Get(outputFolderName.Data());

    if (!merge || directoryConv==0) {
        if (ESDQADirectory)histoVertexZ         = (TH1F*) ESDQADirectory->FindObject("Vertex_Z");
        if (ESDQADirectory)histoContrVertexZ    = (TH1F*) ESDQADirectory->FindObject("ContrVertex_Z");
        ...
        histonSigdEdxElnSigdEdxPosGammaP        = new TH3F("histonSigdEdxElnSigdEdxPosGammaP","", nBinsSigmaTPC, firstBinSigmaTPC, lastBinSigmaTPC, nBinsSigmaTPC, firstBinSigmaTPC,
                                                           lastBinSigmaTPC, nBinsPt, firstBinPt, lastBinPt);  // new sept 7 2015
        ...
        cout << "----> will do the merging in this task" << endl;
    ...
    for (Long64_t i=0; i<nEntriesRecGam;i++) {
        ...
        Float_t nSigmaITSPositron           = (*daugtherProp)[20];
        ...
        Float_t nSigmaITSElectron           = (*daugtherProp)[21];
    ...
    TFile* filePhotoQAWrite = new TFile(fileNameOutput.Data(),"UPDATE");
        ...
        filePhotoQAWrite->mkdir(outputFolderName.Data());
        filePhotoQAWrite->cd(outputFolderName.Data());
