                ...
                if(RootFilePQA->IsZombie()) {
                    ...
                    for (Int_t readFile = 0; readFile < nFilesInList; readFile++ ){
                        ...
                        if (readFile == 0){
                            ...
                            BuildHistogramsForGammaQAAdvV3(fileNamesInList[readFile].Data(),nameCutsPQA,cutTreeProjection,(i>=nData),0,Form("%s/%s/%s/PhotonQA",filePath.Data(), fDataSet.Data(), fRunNumber.Data()),kTRUE);
                            cout << "\t\tdone!" << endl;
                            cout << "\t\t----------------------------------------------------------------------------" << endl;
                        } else {
                            cout << "\t\tCalling BuildHistogramsForGammaQAAdvV3... to add " << fileNamesInList[readFile].Data() << endl;
                            BuildHistogramsForGammaQAAdvV3(fileNamesInList[readFile].Data(),nameCutsPQA,cutTreeProjection,(i>=nData),1,Form("%s/%s/%s/PhotonQA",filePath.Data(), fDataSet.Data(), fRunNumber.Data()),kTRUE);
                            cout << "\t\tdone!" << endl;
                            cout << "\t\t----------------------------------------------------------------------------" << endl;
                        }
                    }
...
//****************************** Create Output ROOT-File ************************************************
    ...
    for(Int_t i=0; i<nSets; i++)
    {
        ...
        else nameOutput = Form("%s_%s/%s/PhotonQA/%s_PhotonQARunwise.root",nameCutsPQA.Data(),cutTreeProjection.Data(),fEnergyFlag.Data(),fDataSet.Data());
