...
void CombineEtaPrimeMeasurements5TeV_pPb( 
    TString fileNamePCM             = "",
    ...
    cout << "dateForOutput: \"" << dateForOutput.Data() << "\"" << endl;

    TString textALICE                           = "ALICE";
    //___________________________________ Declaration of files _____________________________________________
    TString collisionSystem       = "p-Pb, #sqrt{#it{s}} = 5.023 TeV";
    TString outputDir             = Form("%s/%s/CombineEtaPrimeMeasurements%spPb_5.023TeV"                      , suffix.Data(), dateForOutput.Data(), bWCorrection.Data());
    TString outputDirFile         = Form("%s/%s/CombineEtaPrimeMeasurements%spPb_5.023TeV/Inputs"               , suffix.Data(), dateForOutput.Data(), bWCorrection.Data());
    TString outputDirSupportComb  = Form("%s/%s/CombineEtaPrimeMeasurements%spPb_5.023TeV/SupportingCombination", suffix.Data(), dateForOutput.Data(), bWCorrection.Data());
    TString outputDirSupport      = Form("%s/%s/CombineEtaPrimeMeasurements%spPb_5.023TeV/Supporting"           , suffix.Data(), dateForOutput.Data(), bWCorrection.Data());
    TString outputDirSupportPaper = Form("%s/%s/CombineEtaPrimeMeasurements%spPb_5.023TeV/SupportingPaper"      , suffix.Data(), dateForOutput.Data(), bWCorrection.Data());
    ...
    if (!fileNamePCM.EqualTo(""))          gSystem->Exec(Form("cp %s %s/InputPCM.root"        , fileNamePCM.Data()        , outputDirFile.Data()));
    if (!fileNamePCMEMCAL.EqualTo(""))     gSystem->Exec(Form("cp %s %s/InputPCMEMCAL.root"   , fileNamePCMEMCAL.Data()   , outputDirFile.Data()));
    if (!fileNamePCMPHOS.EqualTo(""))      gSystem->Exec(Form("cp %s %s/InputPCMPHOS.root"    , fileNamePCMPHOS.Data()    , outputDirFile.Data()));
    if (!fileNamePHOS.EqualTo(""))         gSystem->Exec(Form("cp %s %s/InputPHOS.root"       , fileNamePHOS.Data()       , outputDirFile.Data()));
    if (!fileNameEMCAL.EqualTo(""))        gSystem->Exec(Form("cp %s %s/InputEMCAL.root"      , fileNameEMCAL.Data()      , outputDirFile.Data()));
    if (!fileNameCorrFactors.EqualTo(""))  gSystem->Exec(Form("cp %s %s/InputCorrFactors.root", fileNameCorrFactors.Data(), outputDirFile.Data()));
    if (!fileNameTheory.EqualTo(""))       gSystem->Exec(Form("cp %s %s/InputTheory.root"     , fileNameTheory.Data()     , outputDirFile.Data()));
    ...
    Color_t  color         = GetColorDefaultColor("pPb_5.023TeV" , ""       , "");
    Color_t  colorMC       = GetColorDefaultColor("pPb_5.023TeV" , "Pythia8", "");
    Style_t  markerStyle   = GetDefaultMarkerStyle("pPb_5.023TeV", ""       , "");
    Style_t  markerStyleMC = GetDefaultMarkerStyle("pPb_5.023TeV", "Pythia8", "");
    Size_t   markerSize    = GetDefaultMarkerSize("pPb_5.023TeV" , ""       , "")*2;
    Size_t   markerSizeMC  = GetDefaultMarkerSize("pPb_5.023TeV" , "Pythia8", "");
    Double_t xSection5TeV = ReturnCorrectXSection("pPb_5.023TeV", 1);; // option is wrong fix when possible
    Double_t xSection5TeVErr = xSection5TeV*1e-3;
    ...
    maxNBinsEtaPrime = GetBinning(xPtLimitsEtaPrime, maxNBinsEtaPrimeAbs, "EtaPrime", "pPb_5.023TeV", 20, -1, kFALSE, "");
    cout << "EtaPrime "<< ": ";
    ...
    // *******************************************************************************************************
    if (!fileNamePCM.EqualTo(""))      fileNamesMethod[0] = fileNamePCM;
    if (!fileNamePHOS.EqualTo(""))     fileNamesMethod[1] = fileNamePHOS;
    if (!fileNameEMCAL.EqualTo(""))    fileNamesMethod[2] = fileNameEMCAL;
    if (!fileNamePCMPHOS.EqualTo(""))  fileNamesMethod[3] = fileNamePCMPHOS;
    if (!fileNamePCMEMCAL.EqualTo("")) fileNamesMethod[4] = fileNamePCMEMCAL;

    // *******************************************************************************************************
    // ***************************** Read in data for different methods **************************************
    // *******************************************************************************************************
    for (Int_t meth = 0; meth < 11; meth++){
        if (!fileNamesMethod[meth].EqualTo("")){
            std::cout << std::endl << "--== Analysing \"" << fileNamesMethod[meth] << "\"" << std::endl;
            ...
            TString dirName;
            switch(meth) {
                case 0: dirName = "EtaPrimepppPb_5.023TeV"; break;
                case 1: dirName = "EtaPrime0-80%pPb_5.023TeV"; break;
                case 2: dirName = "EtaPrime0-80%pPb_5.023TeV"; break;
                case 3: dirName = "EtaPrimeCL1 0-80%pPb_5.023TeV"; break;
                case 4: dirName = "EtaPrimeCL1 0-80%pPb_5.023TeV"; break;
            }
            directoryEtaPrime[meth] = (TDirectory*)fileMethod[meth]->Get(dirName.Data());
            if (!directoryEtaPrime[meth]) {
                cout << "File doesn't contain directory " << dirName << "! Skipping " << nameMeasGlobalLabel[meth].Data()  << endl;
                std::cout << "You could try:" << std::endl;
                fileMethod[meth]->ls();
                continue;
            } else {
                nTotMeasEtaPrime++;
                ...
                graphEtaPrimeEffTimesAcc[meth] = (TGraphAsymmErrors *)directoryEtaPrime[meth]->Get("EffTimesAccEtaPrime");
                    if (graphEtaPrimeEffTimesAcc[meth])
                        graphEtaPrimeEffTimesAcc[meth] = ScaleGraph(graphEtaPrimeEffTimesAcc[meth], branchingRatioEtaPrime[meth]);
    ...
    /*
        ...
        *************************************************************************************************************
        ...
        graphCombEtaPrimeInvYieldTot                 = CombinePtPointsSpectraFullCorrMat(    statErrorCollectionEtaPrime,    sysErrorCollectionEtaPrime,
                                                                                        ...
                                                                                        fileNameEtaPrimeOutputWeighting, "pPb_5.023TeV", "EtaPrime", kTRUE,
        ...
        fitTCMInvYieldEtaPrime        = FitObject("tcm","fitTCMInvYieldEtaPrime5TeVCent","EtaPrime",graphCombEtaPrimeInvYieldTot, minPtEtaPrime, xPtLimitsEtaPrime[maxNBinsEtaPrime],
        ...
        fitPowInvYieldEtaPrime        = FitObject("m","fitPowInvYieldEtaPrime5TeVCent","EtaPrime",graphCombEtaPrimeInvYieldTot,5,40. ,NULL,"QNRMEX0+","", kFALSE);
    ...
    */
    ...
    /*
        ...
        TString nameOutputCommonFile    = Form("CombinedResultsPaperpPb_5.023TeV%s.root", dateForOutput.Data());

        TFile fCombResults(nameOutputCommonFile.Data(), "UPDATE");

        TDirectoryFile* directoryEtaPrimeOutput  = NULL;
        directoryEtaPrimeOutput                  = (TDirectoryFile*)fCombResults.Get(Form("EtaPrimepPb_5.023TeV"));
        if (!directoryEtaPrimeOutput){
            fCombResults.mkdir(Form("EtaPrimepPb_5.023TeV"));
            directoryEtaPrimeOutput              = (TDirectoryFile*)fCombResults.Get(Form("EtaPrimepPb_5.023TeV"));
        }
        fCombResults.cd(Form("EtaPrimepPb_5.023TeV"));
        ...
        TString nameOutputCommonFileFitsOnly    = Form("FitsPaperpPb_5.023TeVCent_%s.root", dateForOutput.Data());
...
int main( int argc, char* argv[] )
{
    ...
    TString import;
    if(argc > 1) fileNamePCM      = argv[1];
    if(argc > 2) fileNameEMCAL    = argv[2];
    if(argc > 3) fileNamePHOS     = argv[3];
    if(argc > 4) fileNamePCMEMCAL = argv[4];
    if(argc > 5) fileNamePCMPHOS  = argv[5];
    if(argc > 6) suffix           = argv[6];
    if(argc > 7) { // isMC
        import = argv[7];
        if( import.EqualTo("kTRUE")  || import.EqualTo("1") ) isMC = kTRUE;
        if( import.EqualTo("kFALSE") || import.EqualTo("0") ) isMC = kFALSE;
    }
    if(argc > 8) bWCorrection        = argv[8];
    if(argc > 9) fileNameCorrFactors = argv[9];

    // Function call ExtractSignalV2
    cout << "Executing \"ProduceFinalResultsPatchedTriggersEtaPrime\" with the following arguments:" << endl
        << "  fileNamePCM:         \"" << fileNamePCM         << "                                \"" << endl
        << "  fileNameEMCAL:       \"" << fileNameEMCAL       << "              \"" << endl
        << "  fileNamePHOS:        \"" << fileNamePHOS        << "                        \"" << endl
        << "  fileNamePCMEMCAL:    \"" << fileNamePCMEMCAL    << "             \"" << endl
        << "  fileNamePCMPHOS:     \"" << fileNamePCMPHOS     << "            \"" << endl
        << "  suffix:              \"" << suffix              << "                             \"" << endl
        << "  isMC:                \"" << isMC                << "                         \"" << endl
        << "  bWCorrection:        \"" << bWCorrection        << "                        \"" << endl
        << "  fileNameCorrFactors: \"" << fileNameCorrFactors << "            \"" << endl
    << endl;
    CombineEtaPrimeMeasurements5TeV_pPb(
        fileNamePCM,
        fileNameEMCAL,
        fileNamePHOS,
        fileNamePCMEMCAL,
