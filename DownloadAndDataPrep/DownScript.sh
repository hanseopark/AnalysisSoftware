...
# Version: V2.1
echo "DownScript.sh Version: V2.1"
...
# 	|-> merge (merges all childs)
# 	|-> runwise (download all runs)
# 	|-> childsareperiods

# Examlple: bash DownScript_V2.sh 776 GA_pp_AOD -GammaConvCalo -RL_listDPGCalo -RL_listDPGEDC -merge -childsareperiods -runwise
...
echo -e "Using Certificat for: $UserName"
...
do
	...
	then
		TrainPage="GA_pp"
	elif [[ $setting = "GA_pp_MC" ]]
	then
		TrainPage="GA_pp_MC"
	elif [[ $setting = "GA_pp_AOD" ]]
	then
		TrainPage="GA_pp_AOD"
	elif [[ $setting = "GA_pp_MC_AOD" ]]
	then
		TrainPage="GA_pp_MC_AOD"
	elif [[ $setting =~ $re ]]
	then
		TrainNumber=${setting}
	elif [[ $setting = "-Name_"* ]]
	...
	elif [[ $setting = "-GammaCalo" ]]
	then
		CutlogInt=4
		Search="GammaCalo"
	elif [[ $setting = "-GammaConvCalo" ]]
	then
		CutlogInt=2
		Search="GammaConvCalo"
	elif [[ $setting = "-GammaConv" ]]
	then
		CutlogInt=0
		Search="GammaConv"
	elif [[ $setting = "-RL_"* ]]
...
if [[ $UseMerge = 1 ]]
then 
	echo -e "\e[1;33m|-> \e[21;39m Merge trending files if any"
fi
echo  -e "\e[1;36m------------------------------------\e[21;39m" 
