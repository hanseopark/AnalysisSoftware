...
if [ $CLEANUPMAYOR == 0 ]; then
    ...
    cd $currentDir
    CopyRunwiseAndMergeAccordingToRunlistData "LHC17n" $HAVELHC17n $OUTPUTDIR_LHC17n $LHC17nData $pathData $baseLegoData "/alice/data/2017" $NSlashes3 runlistsToMerge.txt "pass1" GammaCalo
    cd $currentDir
    CopyRunwiseAndMergeAccordingToRunlistMC "LHC18d2_1" $HAVELHC18d2a $OUTPUTDIR_LHC18d2a $LHC18d2MCa $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMerge.txt GammaCalo
    cd $currentDir
    CopyRunwiseAndMergeAccordingToRunlistMC "LHC18d2_2" $HAVELHC18d2b $OUTPUTDIR_LHC18d2b $LHC18d2MCb $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMerge.txt GammaCalo
    cd $currentDir
    CopyRunwiseAndMergeAccordingToRunlistMC "LHC18d2_3" $HAVELHC18d2c $OUTPUTDIR_LHC18d2c $LHC18d2MCc $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMerge.txt GammaCalo
    cd $currentDir
    CopyRunwiseAndMergeAccordingToRunlistMC "LHC18d2_4" $HAVELHC18d2d $OUTPUTDIR_LHC18d2d $LHC18d2MCd $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMerge.txt GammaCalo


    if [ $HAVELHC17n == 1 ]; then
        ls $OUTPUTDIR_LHC17n/GammaCalo-all_*.root > fileLHC17n.txt
        fileNumbers=`cat fileLHC17n.txt`
        for fileName in $fileNumbers; do
            echo $fileName
            ChangeStructureIfNeededCalo $fileName $OUTPUTDIR_LHC17n $NSlashes "LHC17n-pass$passNr-all" "-all"
        done;
    fi

    if [ $HAVELHC18d2a == 1 ]; then
        ls $OUTPUTDIR_LHC18d2a/GammaCalo-all_*.root > fileLHC18d2a.txt
        fileNumbers=`cat fileLHC18d2a.txt`
        for fileName in $fileNumbers; do
            echo $fileName
            ChangeStructureIfNeededCalo $fileName $OUTPUTDIR_LHC18d2a $NSlashes "MC_LHC18d2-4-all" "-all"
        done;
    fi
    if [ $HAVELHC18d2b == 1 ]; then
        ls $OUTPUTDIR_LHC18d2b/GammaCalo-all_*.root > fileLHC18d2b.txt
        fileNumbers=`cat fileLHC18d2b.txt`
        for fileName in $fileNumbers; do
            echo $fileName
            ChangeStructureIfNeededCalo $fileName $OUTPUTDIR_LHC18d2b $NSlashes "MC_LHC18d2-1-all" "-all"
        done;
    fi
    if [ $HAVELHC18d2c == 1 ]; then
        ls $OUTPUTDIR_LHC18d2c/GammaCalo-all_*.root > fileLHC18d2c.txt
        fileNumbers=`cat fileLHC18d2c.txt`
        for fileName in $fileNumbers; do
            echo $fileName
            ChangeStructureIfNeededCalo $fileName $OUTPUTDIR_LHC18d2c $NSlashes "MC_LHC18d2-2-all" "-all"
        done;
    fi
    if [ $HAVELHC18d2d == 1 ]; then
        ls $OUTPUTDIR_LHC18d2d/GammaCalo-all_*.root > fileLHC18d2d.txt
        fileNumbers=`cat fileLHC18d2d.txt`
        for fileName in $fileNumbers; do
            echo $fileName
            ChangeStructureIfNeededCalo $fileName $OUTPUTDIR_LHC18d2d $NSlashes "MC_LHC18d2-3-all" "-all"
        done;
    fi

    if [ $MERGEON == 1 ]; then
        ls $OUTPUTDIR/GammaCalo_MC_LHC18d2-4-all\_*.root > filesForMerging.txt
        ...
        for fileName in $filesForMerging; do
            ...
            fileA="$OUTPUTDIR/GammaCalo_MC_LHC18d2-4-all""_$number.root"
            fileB="$OUTPUTDIR/GammaCalo_MC_LHC18d2-1-all""_$number.root"
            fileC="$OUTPUTDIR/GammaCalo_MC_LHC18d2-2-all""_$number.root"
            fileD="$OUTPUTDIR/GammaCalo_MC_LHC18d2-3-all""_$number.root"
            echo -e "$fileA\n$fileB\n$fileC\n$fileD" > listCurrMerge.txt
            MergeAccordingToList listCurrMerge.txt $OUTPUTDIR/GammaCalo_MC_LHC18d2x-all\_$number.root
        done
    fi
else
    if [ $HAVELHC17n == 1 ]; then
