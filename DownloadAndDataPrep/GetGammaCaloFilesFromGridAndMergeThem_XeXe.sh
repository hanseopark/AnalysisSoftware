    ...
    if [ $HAVELHC17n == 1 ]; then
        ...
        if [ $SINGLERUN == 1 ]; then
            ...
            if [ $MERGEONSINGLEData == 1 ] && [ ! -f $OUTPUTDIR_LHC17n/mergedAllConv.txt ]; then
                ...
                MergeAccordingToSpecificRunlist fileLHC17n.txt $OUTPUTDIR_LHC17n $NSlashes3 GammaCalo All runlists/runNumbersLHC17n_all.txt "no"
            fi
        else
    ...
    if [ $HAVELHC17j7 == 1 ]; then
        ...
        if [ $SINGLERUN == 1 ]; then
            ...
            if [ $MERGEONSINGLEMC == 1 ] && [ ! -f $OUTPUTDIR_LHC17j7/mergedAllConv.txt ]; then
                ...
                MergeAccordingToSpecificRunlist fileLHC17j7.txt $OUTPUTDIR_LHC17j7 $NSlashes3 GammaCalo All runlists/runNumbersLHC17j7_all.txt "no"
            fi
        else
