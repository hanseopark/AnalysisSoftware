void ChangeStructureToStandardMaterial(TString nameInputFile, TString namefileOutput, TString nameInputList){
   ...
   TList *listOutput =(TList*)fileOutput->Get("GammaConvMaterial");
   Bool_t kNewList = kFALSE;
   if (!listOutput){
      kNewList = kTRUE;
      listOutput = new TList();
      listOutput->SetName("GammaConvMaterial");
   }
