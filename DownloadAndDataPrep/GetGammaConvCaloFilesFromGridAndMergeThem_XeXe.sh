...
if [ $CLEANUPMAYOR == 0 ]; then
    if [ $HAVELHC17n == 1 ]; then
        ...
        if [ $SINGLERUN == 1 ]; then
            ...
            if [ $MERGEONSINGLEData == 1 ] && [ ! -f $OUTPUTDIR_LHC17n/mergedAllConv.txt ]; then
                ...
                MergeAccordingToSpecificRunlist fileLHC17n.txt $OUTPUTDIR_LHC17n $NSlashes3 GammaConvCalo All runlists/runNumbersLHC17n_all.txt "no"
            fi
        else
    ...
    if [ $HAVELHC17j7 == 1 ]; then
        ...
        if [ $SINGLERUN == 1 ]; then
            ...
            if [ $MERGEONSINGLEMC == 1 ] && [ ! -f $OUTPUTDIR_LHC17j7/mergedAllConv.txt ]; then
                cd $currentDir
                rm $OUTPUTDIR_LHC17j7/GammaConvCalo*.root*
                echo runlists/runNumbersLHC17j7_all.txt
                firstrunNumber=`head -n1 runlists/runNumbersLHC17j7_all.txt`
                echo $firstrunNumber
                ls $OUTPUTDIR_LHC17j7/$firstrunNumber/GammaConvCalo_*.root > fileLHC17j7.txt
                MergeAccordingToSpecificRunlist fileLHC17j7.txt $OUTPUTDIR_LHC17j7 $NSlashes3 GammaConvCalo All runlists/runNumbersLHC17j7_all.txt "no"
            fi
        else
    ...
    if [ $HAVELHC17j7 == 1 ]; then
        ls $OUTPUTDIR_LHC17j7/GammaConvCalo-All_*.root > fileLHC17j7.txt
        fileNumbers=`cat fileLHC17j7.txt`
        for fileName in $fileNumbers; do
            echo $fileName
            ChangeStructureIfNeededPCMCalo $fileName $OUTPUTDIR_LHC17j7 $NSlashes "MC_LHC17j7-All" "-All"
        done;
    fi
    ...
    if [ $HAVELHC17j7 == 1 ]; then
        ...
        rm $OUTPUTDIR_LHC17j7/*/GammaConvCalo_*.root
    fi
fi
