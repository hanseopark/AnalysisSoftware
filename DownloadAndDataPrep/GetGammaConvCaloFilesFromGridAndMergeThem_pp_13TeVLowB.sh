...
if [ $CLEANUPMAYOR == 0 ]; then
    if [ $REMERGE == 1 ]; then
        echo -e "DPGTrackIncAccTPCandEMC" > runlistsToMergeLowB.txt
#         echo -e "DPGTrack\nDPGTrackIncAccTPC\nDPGTrackAndCalo\nDPGTrackIncAccTPCandEMC" > runlistsToMergeLowB.txt
        CopyRunwiseAndMergeAccordingToRunlistData "LHC16f" $HAVELHC16f $OUTPUTDIR_LHC16f $LHC16fData $pathData $baseLegoData "/alice/data/2016" $NSlashes3 runlistsToMergeLowB.txt "pass1_lowB" GammaConvCalo
        CopyRunwiseAndMergeAccordingToRunlistData "LHC17g" $HAVELHC17g $OUTPUTDIR_LHC17g $LHC17gData $pathData $baseLegoData "/alice/data/2017" $NSlashes3 runlistsToMergeLowB.txt "pass1" GammaConvCalo
        CopyRunwiseAndMergeAccordingToRunlistData "LHC18c" $HAVELHC18cwoSDD $OUTPUTDIR_LHC18cwoSDD $LHC18cwoSDDData $pathData2 $baseLegoData "/alice/data/2018" $NSlashes3 runlistsToMergeLowB.txt "pass1_CENT_woSDD" GammaConvCalo
        CopyRunwiseAndMergeAccordingToRunlistData "LHC18c" $HAVELHC18cfast $OUTPUTDIR_LHC18cfast $LHC18cfastData $pathData3 $baseLegoData "/alice/data/2018" $NSlashes3 runlistsToMergeLowB.txt "pass1_FAST" GammaConvCalo

        currentDir=$PWD
        echo -e "DPGTrackIncAccTPCandEMC" > runlistsToMergeLowB.txt
#         echo -e "DPGTrack\nDPGTrackIncAccTPC\nDPGTrackAndCalo\nDPGTrackIncAccTPCandEMC" > runlistsToMergeLowB.txt
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC17d1" $HAVELHC17d1 $OUTPUTDIR_LHC17d1 $LHC17d1MC $pathMC $baseLegoMC "/alice/sim/2017" $NSlashes3 runlistsToMergeLowB.txt GammaConvCalo "_lowB"
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC17h3" $HAVELHC17h3 $OUTPUTDIR_LHC17h3 $LHC17h3MC $pathMC $baseLegoMC "/alice/sim/2017" $NSlashes3 runlistsToMergeLowB.txt GammaConvCalo
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC18h1_cent_woSDD" $HAVELHC18h1woSDD $OUTPUTDIR_LHC18h1woSDD $LHC18h1woSDDMC $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMergeLowB.txt GammaConvCalo
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC18h1_fast" $HAVELHC18h1fast $OUTPUTDIR_LHC18h1fast $LHC18h1fastMC $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMergeLowB.txt GammaConvCalo
    fi
...
#     echo -e "DPGTrack\nDPGTrackIncAccTPC\nDPGTrackAndCalo\nDPGTrackIncAccTPCandEMC" > runlistsToMergeLowB.txt
    ...
    for runListName in $listsToMerge; do
        ...
        if [ $HAVELHC17g == 1 ]; then
            ls $OUTPUTDIR_LHC17g/GammaConvCalo-$runListName\_*.root > fileLHC17g.txt
            fileNumbers=`cat fileLHC17g.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCMCalo $fileName $OUTPUTDIR_LHC17g $NSlashes "LHC17g-pass$passNr-$runListName" "-$runListName"
            done;
        fi
        if [ $HAVELHC18cwoSDD == 1 ]; then
            ls $OUTPUTDIR_LHC18cwoSDD/GammaConvCalo-$runListName\_*.root > fileLHC18cwoSDD.txt
            fileNumbers=`cat fileLHC18cwoSDD.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCMCalo $fileName $OUTPUTDIR_LHC18cwoSDD $NSlashes "LHC18c_woSDD-pass$passNr-$runListName" "-$runListName"
            done;
        fi
        if [ $HAVELHC18cfast == 1 ]; then
            ls $OUTPUTDIR_LHC18cfast/GammaConvCalo-$runListName\_*.root > fileLHC18cfast.txt
            fileNumbers=`cat fileLHC18cfast.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCMCalo $fileName $OUTPUTDIR_LHC18cfast $NSlashes "LHC18c_fast-pass$passNr-$runListName" "-$runListName"
            done;
        fi
    done
...
#     echo -e "DPGTrack\nDPGTrackIncAccTPC\nDPGTrackAndCalo\nDPGTrackIncAccTPCandEMC" > runlistsToMergeLowB.txt
    ...
    for runListName in $listsToMerge; do
        ...
        if [ $HAVELHC17h3 == 1 ]; then
            ls $OUTPUTDIR_LHC17h3/GammaConvCalo-$runListName\_*.root > fileLHC17h3.txt
            fileNumbers=`cat fileLHC17h3.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCMCalo $fileName $OUTPUTDIR_LHC17h3 $NSlashes "MC_LHC17h3-anchor17g-$runListName" "-$runListName"
            done;
        fi
        # MC for LHC18cwoSDD
        if [ $HAVELHC18h1woSDD == 1 ]; then
            ls $OUTPUTDIR_LHC18h1woSDD/GammaConvCalo-$runListName\_*.root > fileLHC18h1woSDD.txt
            fileNumbers=`cat fileLHC18h1woSDD.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCMCalo $fileName $OUTPUTDIR_LHC18h1woSDD $NSlashes "MC_LHC18h1_woSDD-anchor18c-$runListName" "-$runListName"
            done;
        fi
        # MC for LHC18cfast
        if [ $HAVELHC18h1fast == 1 ]; then
            ls $OUTPUTDIR_LHC18h1fast/GammaConvCalo-$runListName\_*.root > fileLHC18h1fast.txt
            fileNumbers=`cat fileLHC18h1fast.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCMCalo $fileName $OUTPUTDIR_LHC18h1fast $NSlashes "MC_LHC18h1_fast-anchor18c-$runListName" "-$runListName"
            done;
        fi
    done
...
#             echo -e "DPGTrack\nDPGTrackIncAccTPC\nDPGTrackAndCalo\nDPGTrackIncAccTPCandEMC" > runlistsToMergeLowB.txt
            listsToMerge=`cat runlistsToMergeLowB.txt`
            for runListName in $listsToMerge; do
                ls $OUTPUTDIR/GammaConvCalo_LHC17g-pass$passNr-$runListName\_*.root > filesForMerging.txt
                ...
                for fileName in $filesForMerging; do
                    ...
                    for periodID in $periodList; do
                        echo $periodID
                        currFile=$OUTPUTDIR/GammaConvCalo_LHC$periodID-$runListName\_$number.root
                        if [ -f $currFile ]; then
                    ...
                    MergeAccordingToList listCurrMerge.txt $OUTPUTDIR/GammaConvCalo_LHC$nameOut\pass$passNr-$runListName\_$number.root
                done
                periodList=`echo -e "16f-pass1\n17g-pass1"`
...
#             echo -e "DPGTrack\nDPGTrackIncAccTPC" > runlistsToMergeLowB.txt
            listsToMerge=`cat runlistsToMergeLowB.txt`
            for runListName in $listsToMerge; do
                ls $OUTPUTDIR/GammaConvCalo_MC_LHC17h3-anchor17g-$runListName\_*.root > filesForMerging.txt
                filesForMerging=`cat filesForMerging.txt`
                ...
                for fileName in $filesForMerging; do
                    ...
                    for periodID in $periodList; do
                        echo $periodID
                        currFile=$OUTPUTDIR/GammaConvCalo_MC_LHC$periodID-$runListName\_$number.root
                        if [ -f $currFile ]; then
                            outAdd=`echo $periodID  | cut -d "-" -f 1 `
                            nameOut+=$outAdd"-"
                            echo -e "$currFile\n" >> listCurrMerge.txt
                    ...
                    MergeAccordingToList listCurrMerge.txt $OUTPUTDIR/GammaConvCalo_MC_LHC$nameOut$runListName\_$number.root
                done
          done
        fi
    ...
    if [ $HAVELHC16f == 1 ]; then
        ...
        rm $OUTPUTDIR_LHC16f/*/GammaConvCalo_*.root
        rm $OUTPUTDIR_LHC16f/*/*/*GammaConvCalo_*.root
    fi
fi
