...
if [ $CLEANUPMAYOR == 0 ]; then
    if [ $REMERGE == 1 ]; then
        echo -e "DPGTrack\nDPGTrackIncAccTPC" > runlistsToMerge.txt
        CopyRunwiseAndMergeAccordingToRunlistData "LHC16f" $HAVELHC16f $OUTPUTDIR_LHC16f $LHC16fData $pathData $baseLegoData "/alice/data/2016" $NSlashes3 runlistsToMerge.txt "pass1_lowB" GammaConvV1
        CopyRunwiseAndMergeAccordingToRunlistData "LHC17g" $HAVELHC17g $OUTPUTDIR_LHC17g $LHC17gData $pathData $baseLegoData "/alice/data/2017" $NSlashes3 runlistsToMerge.txt "pass1" GammaConvV1
        CopyRunwiseAndMergeAccordingToRunlistData "LHC18c" $HAVELHC18cwoSDD $OUTPUTDIR_LHC18cwoSDD $LHC18cwoSDDData $pathData2 $baseLegoData "/alice/data/2018" $NSlashes3 runlistsToMerge.txt "pass1_CENT_woSDD" GammaConvV1
        CopyRunwiseAndMergeAccordingToRunlistData "LHC18c" $HAVELHC18cfast $OUTPUTDIR_LHC18cfast $LHC18cfastData $pathData3 $baseLegoData "/alice/data/2018" $NSlashes3 runlistsToMerge.txt "pass1_FAST" GammaConvV1

        currentDir=$PWD
        echo -e "DPGTrack\nDPGTrackIncAccTPC" > runlistsToMerge.txt
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC17d1" $HAVELHC17d1 $OUTPUTDIR_LHC17d1 $LHC17d1MC $pathMC $baseLegoMC "/alice/sim/2017" $NSlashes3 runlistsToMerge.txt GammaConvV1 "_lowB"
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC17h3" $HAVELHC17h3 $OUTPUTDIR_LHC17h3 $LHC17h3MC $pathMC $baseLegoMC "/alice/sim/2017" $NSlashes3 runlistsToMerge.txt GammaConvV1
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC18h1_cent_woSDD" $HAVELHC18h1woSDD $OUTPUTDIR_LHC18h1woSDD $LHC18h1woSDDMC $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMerge.txt GammaConvV1
        CopyRunwiseAndMergeAccordingToRunlistMC "LHC18h1_fast" $HAVELHC18h1fast $OUTPUTDIR_LHC18h1fast $LHC18h1fastMC $pathMC $baseLegoMC "/alice/sim/2018" $NSlashes3 runlistsToMerge.txt GammaConvV1
    fi
    ...
    for runListName in $listsToMerge; do
        ...
        if [ $HAVELHC17g == 1 ]; then
            ls $OUTPUTDIR_LHC17g/GammaConvV1-$runListName\_*.root > fileLHC17g.txt
            fileNumbers=`cat fileLHC17g.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCM $fileName $OUTPUTDIR_LHC17g $NSlashes "LHC17g-pass$passNr-$runListName" "-$runListName"
            done;
        fi
        if [ $HAVELHC18cwoSDD == 1 ]; then
            ls $OUTPUTDIR_LHC18cwoSDD/GammaConvV1-$runListName\_*.root > fileLHC18cwoSDD.txt
            fileNumbers=`cat fileLHC18cwoSDD.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCM $fileName $OUTPUTDIR_LHC18cwoSDD $NSlashes "LHC18c_woSDD-pass$passNr-$runListName" "-$runListName"
            done;
        fi
        if [ $HAVELHC18cfast == 1 ]; then
            ls $OUTPUTDIR_LHC18cfast/GammaConvV1-$runListName\_*.root > fileLHC18cfast.txt
            fileNumbers=`cat fileLHC18cfast.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCM $fileName $OUTPUTDIR_LHC18cfast $NSlashes "LHC18c_fast-pass$passNr-$runListName" "-$runListName"
            done;
        fi
    done
    ...
    for runListName in $listsToMerge; do
        ...
        if [ $HAVELHC17h3 == 1 ]; then
            ls $OUTPUTDIR_LHC17h3/GammaConvV1-$runListName\_*.root > fileLHC17h3.txt
            fileNumbers=`cat fileLHC17h3.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCM $fileName $OUTPUTDIR_LHC17h3 $NSlashes "MC_LHC17h3-anchor17g-$runListName" "-$runListName"
            done;
        fi
        # MC for LHC18cwoSDD
        if [ $HAVELHC18h1woSDD == 1 ]; then
            ls $OUTPUTDIR_LHC18h1woSDD/GammaConvV1-$runListName\_*.root > fileLHC18h1woSDD.txt
            fileNumbers=`cat fileLHC18h1woSDD.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCM $fileName $OUTPUTDIR_LHC18h1woSDD $NSlashes "MC_LHC18h1_woSDD-anchor18c-$runListName" "-$runListName"
            done;
        fi
        # MC for LHC18cfast
        if [ $HAVELHC18h1fast == 1 ]; then
            ls $OUTPUTDIR_LHC18h1fast/GammaConvV1-$runListName\_*.root > fileLHC18h1fast.txt
            fileNumbers=`cat fileLHC18h1fast.txt`
            for fileName in $fileNumbers; do
                echo $fileName
                ChangeStructureIfNeededPCM $fileName $OUTPUTDIR_LHC18h1fast $NSlashes "MC_LHC18h1_fast-anchor18c-$runListName" "-$runListName"
            done;
        fi
    done
            ...
            for runListName in $listsToMerge; do
                ...
                for fileName in $filesForMerging; do
                    ...
                    for periodID in $periodList; do
                        echo $periodID
                        currFile=$OUTPUTDIR/GammaConvV1_LHC$periodID-$runListName\_$number.root
                        if [ -f $currFile ]; then
                    ...
                    MergeAccordingToList listCurrMerge.txt $OUTPUTDIR/GammaConvV1_LHC$nameOut\pass$passNr-$runListName\_$number.root
                done
                periodList=`echo -e "16f-pass1\n17g-pass1"`
        ...
        if [ $MERGEONMC == 1 ]; then
            ...
            for runListName in $listsToMerge; do
                ...
                for fileName in $filesForMerging; do
                    ...
                    for periodID in $periodList; do
                        echo $periodID
                        currFile=$OUTPUTDIR/GammaConvV1_MC_LHC$periodID-$runListName\_$number.root
                        if [ -f $currFile ]; then
                            outAdd=`echo $periodID  | cut -d "-" -f 1 `
                            nameOut+=$outAdd"-"
                            echo -e "$currFile\n" >> listCurrMerge.txt
                    ...
                    MergeAccordingToList listCurrMerge.txt $OUTPUTDIR/GammaConvV1_MC_LHC$nameOut$runListName\_$number.root
                done
          done
        fi
    ...
    if [ $HAVELHC16f == 1 ]; then
        ...
        rm $OUTPUTDIR_LHC16f/*/GammaConvV1_*.root
        rm $OUTPUTDIR_LHC16f/*/*/*GammaConvV1_*.root
    fi
fi
