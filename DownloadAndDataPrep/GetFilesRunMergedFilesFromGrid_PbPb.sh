#! /bin/bash
BASEDIR=/run/media/fredi/ff316d3c-0ff4-40e6-8b73-e06f46108e1b/Grid/OutputLegoTrains/PbPb
# BASEDIR=/run/media/fredi/422118e5-8d1a-4cfc-ba0a-a12a0cb43bee/Photon/Grid/OutputLegoTrains
mkdir -p $BASEDIR
...
fileName=GammaConvV1_QA_568000106009266304480300000.root
...
#    
   ...
   runNumbers=`cat mergeFailedLHC13d2.txt`
   for runNumber in $runNumbers; do
      echo $runNumber
      mkdir -p $OUTPUTDIR_LHC13d2/$runNumber/Stage1
#       nFiles=`ls $OUTPUTDIR_LHC13d2/$runNumber/Stage1 | wc -l`
#       if [ $nFiles = $nFilesToCopy ]; then
#          echo "files copied alread for run " $runNumber
#       if [ -f $OUTPUTDIR_LHC13d2/$runNumber/$fileName ]; then
#            echo "file " $fileName  " has already been copied for run " $runNumber
#       else  
         alien_cp alien:/alice/sim/2013/LHC13d2/$runNumber/PWGGA/$TRAINPATHMC/$LHC13d2/Stage_1/* file:$OUTPUTDIR_LHC13d2/$runNumber/Stage1
#       fi
   done;
