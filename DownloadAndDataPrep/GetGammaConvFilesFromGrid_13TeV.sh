...
# This macro copies GammaConvV1_<trainconfig>.root files from grid and puts them in the correct directories

#****** arguments ******************************************************************************************************************
# $1 = period 
# $2 = "runwise" (download runwise and optional merge afterwards) / 
#      "merge"   (merge ONLY)

#***** variables to be set manually ******************************************************************************************************************
# specify name of runlist on grid:
RUNLIST=merge

# specify where the files should be saved:
# will be $BASEDIR/$SYSTEM/$PERIOD/$RUNLIST/GammaConvV1_<trainconfig>.root
#         $BASEDIR/$SYSTEM/$PERIOD/$run/GammaConvV1_<trainconfig>.root
BASEDIR=/home/meike/Data/GridOutput/GridOutputGammaConv
...
WriteInfo() {
	...
	if [ "$answerInfo" == "yes" ]; then
	   echo -e "$TRAIN\n$PERIOD\n$PASS" > $BASEDIR/$SYSTEM/$PERIOD/info.txt  # write also ESD/AOD
...
if [ $2 = "runwise" ]; then
    ...
    echo "second argument should be runwise, merged or merge";
fi

    #******* SELECT TRAIN **********          if not $2 = merged!
    ...
    #check if there is already data for this period        if not $2 = merged!!
    OldData
	...
	for run in "${RUN[@]}"; do
	    ...
	    if [ "$EXISTS" != "" ] && [ "$answer" != "all" ] && [ "$answer" != "none" ]; then
	        echo "files already exist for this run: "$EXISTS" download anyways? yes / all / no / none";
		read answer
