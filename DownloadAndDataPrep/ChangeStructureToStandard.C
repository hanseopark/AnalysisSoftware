void ChangeStructureToStandard(TString nameInputFile, TString namefileOutput, TString nameInputList){
   ...
   TList *listOutput =(TList*)fileOutput->Get("GammaConvV1");
   Bool_t kNewList = kFALSE;
   if (!listOutput){
      kNewList = kTRUE;
      listOutput = new TList();
      listOutput->SetName("GammaConvV1");
   }
