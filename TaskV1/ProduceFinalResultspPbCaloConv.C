...
void ProduceFinalResultspPbCaloConv(TString fileName = "myOutput", TString cutSelection = "", TString suffix = "gif", TString textMeson = "Pi0" ,TString makeBinShiftWithFunction = "", TString optionEnergy = "", TString fileName2= "", Bool_t optNoBinShift=kFALSE, TString optDalitz = "",Int_t mode = 1){
	gROOT->Reset();	
	...
	Int_t offsetSyst 						= 2;
	Bool_t kMeson 							= kTRUE;
	fileNameSysErr 							= Form("SystematicErrorsNew/SystematicErrorAveragedCaloConv_DummypPb.dat"); 
	...
	TH1D* histoCorrectedYieldNormEff 		= (TH1D*)file->Get("CorrectedYieldNormEff"); 
	cout << "Bins" << endl;
	...
	TH1D* histoFWHMMesonMCrec 				= (TH1D*)file->Get("histoFWHMMesonRecMC");    
	TH1D* histoMassMesonMCrec 				= (TH1D*)file->Get("histoMassMesonRecMC");
	TH1D* histoWidthGaussianMeson 			= (TH1D*)file->Get("histoWidthGaussianMeson");    
	TH1D* histoMassGaussianMeson 			= (TH1D*)file->Get("histoMassGaussianMeson");
	TH1D* histoTrueWidthGaussianMeson 		= (TH1D*)file->Get("histoTrueWidthGaussianMeson");    
	TH1D* histoTrueMassGaussianMeson 		= (TH1D*)file->Get("histoTrueMassGaussianMeson");
	TH1D* histoWidthGaussianMesonMCrec 		= (TH1D*)file->Get("histoWidthGaussianMesonRecMC");    
	TH1D* histoMassGaussianMesonMCrec 		= (TH1D*)file->Get("histoMassGaussianMesonRecMC");
...
// 	TH1D* histoBGEstimateFromPileup 		= (TH1D*)file->Get("BGEstimateFromPileup");
	...
	TH1D* histoFWHMMesonMCRecMeV 			= (TH1D*) histoFWHMMesonMCrec->Clone();
	histoFWHMMesonMCRecMeV->Scale(1000.);

	TH1D* histoWidthGaussianMesonMeV 		= NULL;
	if (histoWidthGaussianMeson){
		histoWidthGaussianMesonMeV 				= (TH1D*) histoWidthGaussianMeson->Clone();
		histoWidthGaussianMesonMeV->Scale(1000.);	
	}
	
	TH1D* histoTrueWidthGaussianMesonMeV 	= NULL;
	if (histoTrueWidthGaussianMeson){
		histoTrueWidthGaussianMesonMeV 			= (TH1D*) histoTrueWidthGaussianMeson->Clone();
		histoTrueWidthGaussianMesonMeV->Scale(1000.);
	}
	TH1D* histoWidthGaussianMesonMCrecMeV 	= NULL;
	if (histoWidthGaussianMeson){
		histoWidthGaussianMesonMCrecMeV 		= (TH1D*) histoWidthGaussianMeson->Clone();
		histoWidthGaussianMesonMCrecMeV->Scale(1000.);
	}
...
// 	}
	...
	TGraphAsymmErrors* graphCorrectedYieldNormalEffSysErr 		= CalculateSysErrFromRelSysHisto( histoCorrectedYieldNormEff, Form("%sNormalEffSystError",textMeson.Data()),relSystErrorDown , relSystErrorUp, offsetSyst, nPoints);
	TGraphAsymmErrors* graphCorrectedYieldNormalEffSysErrA 		= CalculateSysErrFromRelSysHisto( histoCorrectedYieldNormEff, Form("%sNormalEffSystErrorA",textMeson.Data()),relSystErrorWOMaterialDown , relSystErrorWOMaterialUp, offsetSyst, nPoints);
	TGraphAsymmErrors* graphCorrectedYieldNormalEffStatPlusSys 	= CalculateSysErrFromRelSysHistoComplete( histoCorrectedYieldNormEff, Form("%sNormalEffComplError",textMeson.Data()),relSystErrorDown , relSystErrorUp, offsetSyst, nPoints);
...
// 		canvasRAA->SaveAs(Form("%s/%s_%s_RAA_%s.%s", outputDir.Data(), textMeson.Data() ,prefix2.Data(), cutSelection.Data(), suffix.Data()));
	...
	if (fileName2.CompareTo("")!=0 && textMeson.CompareTo("Eta")==0){
		...
		fileNameSysErrPi0EtaBinning 					= Form("SystematicErrorsNew/SystematicErrorAveragedCaloConv_DummypPb.dat"); 
		histoCorrectedYieldEta 							= (TH1D*) histoCorrectedYield->Clone(); 
		histoCorrectedYieldEtaNormEff 					= (TH1D*) histoCorrectedYieldNormEff->Clone(); 
		file2 											= new TFile(fileName2);
		histoCorrectedYieldPi0EtaBinning 				= (TH1D*)file2->Get("CorrectedYieldTrueEff"); 
		histoCorrectedYieldPi0EtaBinningNormEff 		= (TH1D*)file2->Get("CorrectedYieldNormEff"); 
		...
		histoRatioEtaPi0NormEff 						= (TH1D*) histoCorrectedYieldEtaNormEff->Clone(); 
		histoRatioEtaPi0NormEff->Divide(histoRatioEtaPi0NormEff,histoCorrectedYieldPi0EtaBinningNormEff,1.,1.,"");
		...
		for (Int_t i = 0; i < nPoints + 1; i++){
			ratioXValueNormEff[i] = histoRatioEtaPi0NormEff->GetBinCenter(i+offsetSyst);
			ratioYValueNormEff[i] = histoRatioEtaPi0NormEff->GetBinContent(i+offsetSyst);
			ratioXErrorNormEff[i] = histoRatioEtaPi0NormEff->GetBinWidth(i+offsetSyst)/2.;
			ratioSysUpErrorNormEff[i]= TMath::Sqrt(TMath::Power(relSystErrorWOMaterialUp[i]/100* histoCorrectedYieldEtaNormEff->GetBinContent(i+offsetSyst)/histoCorrectedYieldPi0EtaBinningNormEff->GetBinContent(i+offsetSyst),2) + TMath::Power(histoCorrectedYieldEtaNormEff->GetBinContent(i+offsetSyst)*relPi0EtaBinningSystErrorWOMaterialUp[i]/100/histoCorrectedYieldPi0EtaBinningNormEff->GetBinContent(i+offsetSyst),2));
			ratioSysDownErrorNormEff[i]= TMath::Sqrt(TMath::Power(relSystErrorWOMaterialDown[i]/100* histoCorrectedYieldEtaNormEff->GetBinContent(i+offsetSyst)/histoCorrectedYieldPi0EtaBinningNormEff->GetBinContent(i+offsetSyst),2) + TMath::Power(histoCorrectedYieldEtaNormEff->GetBinContent(i+offsetSyst)*relPi0EtaBinningSystErrorWOMaterialDown[i]/100/histoCorrectedYieldPi0EtaBinningNormEff->GetBinContent(i+offsetSyst),2));
		}
		
		graphSystErrRatioNormEff = new TGraphAsymmErrors(nPoints,ratioXValueNormEff,ratioYValueNormEff,ratioXErrorNormEff,ratioXErrorNormEff,ratioSysDownErrorNormEff,ratioSysUpErrorNormEff);
		graphSystErrRatioNormEff->SetFillColor(kGray+1);
	...
	TString fileNameOutputComp = Form("%s_%sResults_pPb%s.root",prefix2.Data(),system.Data(),applyBinShift.Data());
	cout << fileNameOutputComp.Data() << endl;
	TFile* fileOutputForComparisonFullyCorrected = new TFile(fileNameOutputComp.Data(),"UPDATE");		
		if (textMeson.CompareTo("Pi0") == 0){
			if (histoNumberOfEvents) 						histoNumberOfEvents->Write(Form("histoNumberOfEvents%s%s",optionEnergy.Data(),centralityString.Data()),TObject::kOverwrite);
		}
		...
		if (histoCorrectedYield)						histoCorrectedYield->Write(Form("CorrectedYield%s",textMeson.Data()),TObject::kOverwrite);
		if (histoCorrectedYieldNormEff)					histoCorrectedYieldNormEff->Write(Form("CorrectedYieldNormEff%s",textMeson.Data()),TObject::kOverwrite);
		if (histoUncorrectedYield)						histoUncorrectedYield->Write(Form("RAWYieldPerEvents%s",textMeson.Data()),TObject::kOverwrite);
		if (histoAcceptance)							histoAcceptance->Write(Form("Acceptance%s",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueEffPt)								histoTrueEffPt->Write(Form("Efficiency%s",textMeson.Data()),TObject::kOverwrite);
		if (histoMCInput)								histoMCInput->Write(Form("%sMCInput",textMeson.Data()),TObject::kOverwrite);
		...
		if (textMeson.CompareTo("Pi0") == 0){
		    if (histoCorrYieldBinShifted) 					histoCorrYieldBinShifted->Write("CorrectedYieldPi0BinShifted",TObject::kOverwrite);
		    if (graphCorrectedYieldSysErrBinShifted) 		graphCorrectedYieldSysErrBinShifted->Write(Form("%sSystErrorBinShifted",textMeson.Data()),TObject::kOverwrite);
		    if (graphCorrectedYieldSysErrABinShifted) 		graphCorrectedYieldSysErrABinShifted->Write(Form("%sSystErrorABinShifted",textMeson.Data()),TObject::kOverwrite);
		    if (graphCorrectedYieldStatPlusSysBinShifted) 	graphCorrectedYieldStatPlusSysBinShifted->Write(Form("%sComplErrorBinShifted",textMeson.Data()),TObject::kOverwrite);
		}
		if (fileName2.CompareTo("")!=0 && textMeson.CompareTo("Eta")==0){
		    ...
		    if (histoRatioEtaPi0NormEff) 					histoRatioEtaPi0NormEff->Write("EtatoPi0RatioNormEff",TObject::kOverwrite); 
		    if (graphSystErrRatioNormEff) 					graphSystErrRatioNormEff->Write("EtatoPi0RatioSysNormEff",TObject::kOverwrite);
		}
		if (histoFWHMMeson) 							histoFWHMMeson->Write(Form("FWHM%s",textMeson.Data()),TObject::kOverwrite);
		if (histoFWHMMesonMeV) 							histoFWHMMesonMeV->Write(Form("FWHM%sMeV",textMeson.Data()),TObject::kOverwrite);
		if (histoFWHMMesonMCrec) 						histoFWHMMesonMCrec->Write(Form("FWHMMCrec%s",textMeson.Data()),TObject::kOverwrite);
		if (histoFWHMMesonMCRecMeV) 					histoFWHMMesonMCRecMeV->Write(Form("FWHMMCrec%sMeV",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueFWHMMeson) 						histoTrueFWHMMeson->Write(Form("TrueFWHM%s",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueFWHMMesonMeV) 						histoTrueFWHMMesonMeV->Write(Form("TrueFWHM%sMeV",textMeson.Data()),TObject::kOverwrite);
		if (histoWidthGaussianMeson) 					histoWidthGaussianMeson->Write(Form("WidthGaussian%s",textMeson.Data()),TObject::kOverwrite);
		if (histoWidthGaussianMesonMeV) 				histoWidthGaussianMesonMeV->Write(Form("WidthGaussian%sMeV",textMeson.Data()),TObject::kOverwrite);
		if (histoWidthGaussianMesonMCrec) 				histoWidthGaussianMesonMCrec->Write(Form("WidthGaussianMCrec%s",textMeson.Data()),TObject::kOverwrite);
		if (histoWidthGaussianMesonMCrecMeV)		 	histoWidthGaussianMesonMCrecMeV->Write(Form("WidthGaussianMCrec%sMeV",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueWidthGaussianMeson) 				histoTrueWidthGaussianMeson->Write(Form("TrueWidthGaussian%s",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueWidthGaussianMesonMeV) 			histoTrueWidthGaussianMesonMeV->Write(Form("TrueWidthGaussian%sMeV",textMeson.Data()),TObject::kOverwrite);
		
		if (histoMassMeson) 							histoMassMeson->Write(Form("Mass%s",textMeson.Data()),TObject::kOverwrite);
		if (histoMassGaussianMeson) 					histoMassGaussianMeson->Write(Form("MassGaussian%s",textMeson.Data()),TObject::kOverwrite);
		if (histoMassMesonMinusExp) 					histoMassMesonMinusExp->Write(Form("Mass%sMinusExp",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueMassMeson) 						histoTrueMassMeson->Write(Form("TrueMass%s",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueMassGaussianMeson) 				histoTrueMassGaussianMeson->Write(Form("TrueMassGaussian%s",textMeson.Data()),TObject::kOverwrite);
		if (histoTrueMassMesonMinusExp) 				histoTrueMassMesonMinusExp->Write(Form("TrueMass%sMinusExp",textMeson.Data()),TObject::kOverwrite);
		if (histoMassMesonMCrec) 						histoMassMesonMCrec->Write(Form("MassMCrec%s",textMeson.Data()),TObject::kOverwrite);
		if (histoMassGaussianMesonMCrec) 				histoMassGaussianMesonMCrec->Write(Form("MassGaussianMCrec%s",textMeson.Data()),TObject::kOverwrite);

		if (graphCorrectedYieldSysErr) 					graphCorrectedYieldSysErr->Write(Form("%sSystError",textMeson.Data()),TObject::kOverwrite);
		if (graphCorrectedYieldSysErrA) 				graphCorrectedYieldSysErrA->Write(Form("%sSystErrorA",textMeson.Data()),TObject::kOverwrite);
		if (graphCorrectedYieldStatPlusSys) 			graphCorrectedYieldStatPlusSys->Write(Form("%sComplError",textMeson.Data()),TObject::kOverwrite);
		if (graphCorrectedYieldNormalEffSysErr) 		graphCorrectedYieldNormalEffSysErr->Write(Form("%sNormalEffSystError",textMeson.Data()),TObject::kOverwrite);
		if (graphCorrectedYieldNormalEffSysErrA) 		graphCorrectedYieldNormalEffSysErrA->Write(Form("%sNormalEffSystErrorA",textMeson.Data()),TObject::kOverwrite);
		if (graphCorrectedYieldNormalEffStatPlusSys) 	graphCorrectedYieldNormalEffStatPlusSys->Write(Form("%sNormalEffComplError",textMeson.Data()),TObject::kOverwrite);
