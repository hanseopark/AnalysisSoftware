...
#include "THnSparse.h"
using namespace std; // for g++ compilation
...
void AnalyseNeutralMesonSignificance(
    ...
    TString autoDetectedMainDir             = AutoDetectMainTList(modeHeavy , fileData);
    ...
    if(HistosGammaConversionData == nullptr){
    ...
    if ( modeHeavy >= 100 ) {
        nameMesonHistoPtY     = "ESD_MotherMeson_Pt_Y";
        nameMesonHistoPtAlpha = "ESD_MotherMeson_Pt_Alpha";
        nameMesonHistoPtOpen  = "ESD_MotherMeson_Pt_OpenAngle";
    }

    TH2F* histoPi0RapidityPtData        = (TH2F*) ESDContainerData->FindObject(nameMesonHistoPtY.Data());
    if(histoPi0RapidityPtData == nullptr){
        cout << "ERROR: \"" << nameMesonHistoPtY << "\" not found in File" << endl;
        return;
    }
    ...
    if(histoPi0AlphaPtData == nullptr){
        cout << "ERROR: \"" << nameMesonHistoPtAlpha << "\" not found in File" << endl;
        return;
    }
    ...
    autoDetectedMainDir             = AutoDetectMainTList(modeHeavy , fileMC);
    if(fileMC == nullptr){
        cout << "ERROR: MC file \"" << fileNameMC << "\" not found" << endl;
        return;
    }
    TList *TopDirMC = (TList*)fileMC->Get(autoDetectedMainDir.Data());
    if(TopDirMC == nullptr){
        cout << "ERROR: TopDirMC \"" << autoDetectedMainDir << "\" not found" << endl;
        return;
    }
    TList *HistosGammaConversionMC = (TList*)TopDirMC->FindObject(Form("Cut Number %s",cutSel.Data()));
    if(HistosGammaConversionMC == nullptr){
        cout << "ERROR: " << Form("Cut Number %s",cutSel.Data()) << " not found in file" << endl;
        return;
    }
    ...
    TH2F* histoTruePi0CaloConvPhotonConvRPtE        = nullptr;
    TH2F* histoTruePi0CaloConvPhotonConvRAlphaE     = nullptr;
    TH2F* histoTruePi0CaloConvPhotonPtEAlphaE       = nullptr;

    TH1D* histoTruePi0CaloConvPhotonPtE_FullR       = nullptr;
    TH1D* histoTruePi0CaloConvPhotonPtE_RL180       = nullptr;
    TH1D* histoTruePi0CaloConvPhotonPtE_RS180       = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_FullR    = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_R180360  = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_R360460  = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_RS180    = nullptr;
    TH1D* histoTruePi0CaloConvPhotonConvR           = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_FullPt   = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_Pt0T500  = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_Pt500T1  = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_Pt1T2    = nullptr;
    TH1D* histoTruePi0CaloConvPhotonAlphaE_PtA2     = nullptr;
    ...
    for (Int_t i = startBinPi0; i < maxNbins; i++){
        ...
        histoRecPi0AlphaPtMCpTBins[i]->Draw("hist,e1,same");
        ...
        legendAlpha->SetLineColor(0);
        legendAlpha->AddEntry(histoTruePi0AlphaPtMCpTBins[i],"True #pi^{0}");
        legendAlpha->AddEntry(histoRecPi0AlphaPtDatapTBins[i],"Rec Data");
        legendAlpha->AddEntry(histoRecPi0AlphaPtMCpTBins[i],"Rec MC");
        legendAlpha->Draw();
    ...
    if (mode != 10){
        ...
        TH2F* histoTrueEtaCaloConvPhotonConvRPtE        = nullptr;
        TH2F* histoTrueEtaCaloConvPhotonConvRAlphaE     = nullptr;
        TH2F* histoTrueEtaCaloConvPhotonPtEAlphaE       = nullptr;
        if (enableExtConvCaloQA){
    ...
    if(listTreeMCCalo != nullptr) enableMesonTree = kTRUE;
    cout << "here" << endl;
    if (enableMesonTree){
...
{
    // Default arguments for AnalyseNeutralMesonSignificance
        TString fileNameData = "myOutput";
        TString fileNameMC   = "myOutput";
        TString cutSel       = "";
        TString suffix       = "gif";
    ...
    // Import main call arguments
        if( argc > 1 ) fileNameData = argv[1];
        if( argc > 2 ) fileNameMC   = argv[2];
        if( argc > 3 ) cutSel       = argv[3];
        if( argc > 4 ) suffix       = argv[4];
        if( argc > 5 ) optEnergy    = argv[5];
        if( argc > 6 ) optPeriod    = argv[6];
        if( argc > 7 ) optGenerator = argv[7];
        if( argc > 8 ) { // mode
            istringstream sstr(argv[8]);
            sstr >> mode;
        }

    // Function call AnalyseNeutralMesonSignificance
        cout << "Executing \"AnalyseNeutralMesonSignificance\" with the following arguments:" << endl
            << "  fileNameData: \"" << fileNameData << "\"" << endl
            << "  fileNameMC:   \"" << fileNameMC   << "\"" << endl
            << "  cutSel:       \"" << cutSel       << "\"" << endl
            << "  suffix:       \"" << suffix       << "\"" << endl
            << "  optEnergy:    \"" << optEnergy    << "\"" << endl
            << "  optPeriod:    \"" << optPeriod    << "\"" << endl
            << "  optGenerator: \"" << optGenerator << "\"" << endl
            << "  mode:         \"" << mode         << "\"" << endl
        << endl;
        AnalyseNeutralMesonSignificance(
            fileNameData,
            fileNameMC,
            cutSel,
            suffix,
