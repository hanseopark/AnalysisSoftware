...
void AnalyseMaterialHistos(
			   ...
			   TString fileNameMC               = "", 
			   //			   Int_t mode                     = 0 , 
			   TString optionPeriod           = "", TString optionGrid="Plots"){
    ...
    Float_t         arrayZBins[13] =                {10.,15.,20.,40.,40.,60.,60.,80.,120.,120.,200.,200.,200};
    Float_t         arrayRBins[13] =                {0.,1.5,5.,8.6,13.,21.,33.5,41.,55.,72.,90.,150.,180};
    ...
    TString outputDirectory= Form("%s/%s",optionGrid.Data(),fGammaCutSelection.Data());
    ...
    TH2F *histoMappingPtData =       (TH2F*)ESDContainer->FindObject("ESD_ConversionMapping_Pt");
    TH2F *histoMappingPt5cmData =       (TH2F*)ESDContainer->FindObject("ESD_ConversionMapping_Pt5cm");
    ...
    TH2F *histoMappingPtMC =         (TH2F*)ESDContainerMC->FindObject("ESD_ConversionMapping_Pt");
    TH2F *histoMappingPt5cmMC =         (TH2F*)ESDContainerMC->FindObject("ESD_ConversionMapping_Pt5cm");
    ...
    canvasNTracks->Print(Form("%s/NumberOfGoodESDTracks_%s.pdf",outputDirectory.Data(),fGammaCutSelection.Data() ));
    ...
    latexCutName->Draw("same");
    canvasPhotonR->Print(Form("%s/PhotonRadius_%s.pdf",outputDirectory.Data(),fGammaCutSelection.Data() ));
  ...
  TCanvas * canvasPhotonPurity = new TCanvas("canvasPhotonPurity","",1200,1000);  // gives the page size
    ...
    linePt->Draw("same");
    latexCutName->Draw("same");
    canvasPhotonPurity->Print(Form("%s/PhotonPurity_%s.pdf",outputDirectory.Data(),fGammaCutSelection.Data() ));
    ...
    latexCutName->Draw("same");
    canvasPhotonChar->Print(Form("%s/PhotonChar_%s.pdf",outputDirectory.Data(),fGammaCutSelection.Data() ));
    ...
    for(Int_t iR = 0; iR < nBinsR; iR++){
      ...
      canvasSingleBin->SaveAs(Form("%s/Phi_in_R_%i_%s.pdf",outputDirectory.Data(), iR,fGammaCutSelection.Data()));
    ...
    for(Int_t iR = 0; iR < nBinsR; iR++){
      ...
      latexCutName->Draw("same");
      canvasSingleBin->Update();
      canvasSingleBin->SaveAs(Form("%s/Z_in_R_%i_%s.pdf",outputDirectory.Data(), iR,fGammaCutSelection.Data()));
      delete canvasSingleBin;
    }
