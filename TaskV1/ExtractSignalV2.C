...
void ExtractSignalV2(   TString meson                   = "", 
    ...
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){ // BEGIN ANALYSIS for each Pt bin
        ...
        if(fCrysFitting==0){
            ...
            if (fFitReco)
                fMesonChi2[iPt]                         = fFitReco->GetChisquare()/fFitReco->GetNDF();
            else 
                fMesonChi2[iPt]                         = -1;
        } else {
            fFileErrLog << "Using Crystal Ball function"<<endl;
...
//                    fMesonIntDeltaRangeNarrow[1]    = fMesonIntDeltaRange[1]*0.4; // mod.
    ...
    fMesonChi2                                                      = new Double_t[fNBinsPt];
...
void CreatePtHistos(){
    ...
    fHistoChi2                          = new TH1D("histoChi2","",fNBinsPt,fBinsPt);
    fHistoChi2->Sumw2();
...
void Delete(){
    ...
    if (fMesonChi2)                                             delete fMesonChi2;
