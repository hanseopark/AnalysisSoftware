...
//         histoMeanMassRatio[i]->GetYaxis()->SetTitleOffset(1.1);
    ...
    for(Int_t i=0; i<nNL; i++){
        ...
        cout << Form("%s_TotalCorr",nameUsedCorr[i].Data()) << endl;
        ...
        if(!fitTotalCorrection[i]){
            ...
            for (Int_t nPar = 0; nPar < fitTotalCorrection[i]->GetNpar(); nPar++){
                cout << nPar << "\t" << fitTotalCorrection[i]->GetParameter(nPar) << endl;
            }
            ...
            for (Int_t k = 0; k < 3; k++){
                if (hasAddInput[k][i]){
                    cout << k << "\t"<< Form("%s_TotalCorr",nameUsedCorrAdd[k][i].Data()) << endl;
                    fitTotalCorrectionAdd[k][i]     = (TF1*)inputFilesAdd[k][i]->Get(Form("%s_TotalCorr",nameUsedCorrAdd[k][i].Data()));
                    if (fitTotalCorrectionAdd[k][i]){
                        for (Int_t nPar = 0; nPar < fitTotalCorrectionAdd[k][i]->GetNpar(); nPar++){
                            cout << nPar << "\t" << fitTotalCorrectionAdd[k][i]->GetParameter(nPar) << endl;
                        }
                    }
