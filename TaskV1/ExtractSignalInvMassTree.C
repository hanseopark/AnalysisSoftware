...
#include "ExtractSignalInvMassTree.h"
...
void ExtractSignalInvMassTree(
    ...
    if(mode == 4 && optionMC.CompareTo("kTRUE") == 0){
        ...
        if(fEventCutSelectionTriggerRead.CompareTo("83")==0     || fEventCutSelectionTriggerRead.CompareTo("85")==0     || fEventCutSelectionTriggerRead.CompareTo("93")==0 ||
            fEventCutSelectionTriggerRead.CompareTo("95")==0    || fEventCutSelectionTriggerRead.CompareTo("52")==0     || fEventCutSelectionTriggerRead.CompareTo("51")==0 ||
            fEventCutSelectionTriggerRead.CompareTo("00")==0){
    ...
    TList *TreeContainer                = (TList*) HistosGammaConversion->FindObject(Form("%s InvMass Tree",fCutSelectionRead.Data()));
    ...
    fGammaGammaInvMassVSPt->Reset("ICES");
    fGammaGammaAlphaVSPt                = new TH2D("alpha","alpha",100,0,1.,250,0.,25.);
    fGammaGammaThetaVSPt                = new TH2D("theta","theta",400,0,0.2,250,0.,25.);
    fGammaGammaInvMassVSAlpha           = new TH3D("invmass_alpha","invmass_alpha",800,0.,0.8,100,0,1.,250,0.,25.);
    fGammaGammaInvMassVSTheta           = new TH3D("invmass_theta","invmass_theta",800,0.,0.8,200,0,0.2,250,0.,25.);

    fBckInvMassVSPt                     = (TH2D*)ESDContainer->FindObject(ObjectNameBck.Data());
    fBckInvMassVSPt->Sumw2();
    fBckInvMassVSPt->Reset("ICES");
    fBckAlphaVSPt                       = new TH2D("bck_alpha","bck_alpha",100,0,1.,250,0.,25.);
    fBckThetaVSPt                       = new TH2D("bck_theta","bck_theta",400,0,0.2,250,0.,25.);
    ...
    //read signal tree
    TTree *sig_tree=(TTree*) TreeContainer->FindObject("Sig_InvMass_Pt_Alpha_Theta_MixPool");
    sig_tree->SetBranchAddress("InvMass",&tInvMass);
    sig_tree->SetBranchAddress("Pt",&tPt);
    sig_tree->SetBranchAddress("Alpha",&tAlpha);
    sig_tree->SetBranchAddress("Theta",&tTheta);
    sig_tree->SetBranchAddress("MixPool",&tMixPool);

    Int_t nTreeEntries=sig_tree->GetEntries();
    Double_t nextStep = 10;
    cout << "signal tree: " << endl;
    for(Int_t iE=0; iE < nTreeEntries; iE++)
    {
        if ( ((Double_t)(iE*100)) / ((Double_t) nTreeEntries) > nextStep)
        {
            printf("...%.0f%%\n",nextStep);
            fflush(stdout);
            nextStep += 10;
        }
        sig_tree->GetEntry(iE);
        if(tTheta < fCutTheta) continue;
        else{
          fGammaGammaInvMassVSPt->Fill(tInvMass,tPt);
          fGammaGammaAlphaVSPt->Fill(tAlpha,tPt);
          fGammaGammaThetaVSPt->Fill(tTheta,tPt);
          fGammaGammaInvMassVSAlpha->Fill(tInvMass,tAlpha,tPt);
          fGammaGammaInvMassVSTheta->Fill(tInvMass,tTheta,tPt);
        }
    }
    cout << endl;

    //read background tree
    TTree *bck_tree=(TTree*) TreeContainer->FindObject("Bck_InvMass_Pt_Alpha_Theta_MixPool");
    bck_tree->SetBranchAddress("InvMass",&tInvMass);
    bck_tree->SetBranchAddress("Pt",&tPt);
    bck_tree->SetBranchAddress("Alpha",&tAlpha);
    bck_tree->SetBranchAddress("Theta",&tTheta);
    bck_tree->SetBranchAddress("MixPool",&tMixPool);

    nTreeEntries=bck_tree->GetEntries();
    nextStep = 10;
    cout << "bck tree: " << endl;
    for(Int_t iE=0; iE < nTreeEntries; iE++)
    {
        ...
        bck_tree->GetEntry(iE);
        if(tTheta < fCutTheta) continue;
        else{
          fBckInvMassVSPt->Fill(tInvMass,tPt);
          fBckAlphaVSPt->Fill(tAlpha,tPt);
          fBckThetaVSPt->Fill(tTheta,tPt);
        }
    }
    cout << endl;
    ...
    FillAlphaHistosArray(fGammaGammaAlphaVSPt);
    FillThetaHistosArray(fGammaGammaThetaVSPt);
    FillAlphaBckHistosArray(fBckAlphaVSPt);
    FillThetaBckHistosArray(fBckThetaVSPt);

    FillInvMassAlphaHistosArray(fGammaGammaInvMassVSAlpha);
    FillInvMassThetaHistosArray(fGammaGammaInvMassVSTheta);
    cout << "line " << __LINE__ << endl;
    ...
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){ // BEGIN ANALYSIS for each Pt bin
        ...
        ProcessEM( fHistoMappingGGAlphaPtBin[iPt], fHistoMappingBackAlphaPtBin[iPt], fBGFitRange);
        fHistoMappingSignalAlphaPtBin[iPt] = fSignal;
        fHistoMappingBackNormAlphaPtBin[iPt] = fBckNorm;
        fHistoMappingSignalAlphaPtBin[iPt]->SetName(Form("fHistoMappingSignalAlpha_in_Pt_Bin%02d",iPt));

        ProcessEM( fHistoMappingGGThetaPtBin[iPt], fHistoMappingBackThetaPtBin[iPt], fBGFitRange);
        fHistoMappingSignalThetaPtBin[iPt] = fSignal;
        fHistoMappingBackNormThetaPtBin[iPt] = fBckNorm;
        fHistoMappingSignalThetaPtBin[iPt]->SetName(Form("fHistoMappingSignalTheta_in_Pt_Bin%02d",iPt));
    ...
    //******************** Data OUTPUTFILE ***************************************************
    const char* fileNameSysErrDat = Form("theta_%.4f/%s/%s/%s_%s_SystematicErrorYieldExtraction_RAWDATA_%s.dat",fCutTheta,fCutSelection.Data(),fEnergyFlag.Data(),fPrefix.Data(),fPrefix2.Data(), cutSelection.Data());
    ...
    nameMeson   = Form("%s_MesonAlpha%s", plotPrefix.Data(), plotSuffix.Data());
    nameCanvas  = "MesonAlphaCanvas";
    namePad     = "MesonAlphaPad";
    cout << nameMeson.Data() << endl;
    PlotAlphaInPtBins( fHistoMappingGGAlphaPtBin, fHistoMappingBackNormAlphaPtBin, nameMeson, nameCanvas, namePad, fMesonMassPlotRange, fdate, fPrefix, fRow, fColumn, fStartPtBin, fNBinsPt,
                        fBinsPt, fTextMeasurement, fIsMC ,fDecayChannel, fDetectionProcess, fCollisionSystem);

    nameMeson   = Form("%s_MesonTheta%s", plotPrefix.Data(), plotSuffix.Data());
    nameCanvas  = "MesonThetaCanvas";
    namePad     = "MesonThetaPad";
    cout << nameMeson.Data() << endl;
    PlotThetaInPtBins( fHistoMappingGGThetaPtBin, fHistoMappingBackNormThetaPtBin, nameMeson, nameCanvas, namePad, fMesonMassPlotRange, fdate, fPrefix, fRow, fColumn, fStartPtBin, fNBinsPt,
                        fBinsPt, fTextMeasurement, fIsMC ,fDecayChannel, fDetectionProcess, fCollisionSystem);

    nameMeson   = Form("%s_Meson2DAlpha%s", plotPrefix.Data(), plotSuffix.Data());
    nameCanvas  = "Meson2DAlphaCanvas";
    namePad     = "Meson2DAlphaPad";
    cout << nameMeson.Data() << endl;
    Plot2DInPtBins( fHistoMappingGGInvMassAlphaPtBin, "#alpha", nameMeson, nameCanvas, namePad, fMesonMassPlotRange, fdate, fPrefix, fRow, fColumn, fStartPtBin, fNBinsPt,
                        fBinsPt, fTextMeasurement, fIsMC ,fDecayChannel, fDetectionProcess, fCollisionSystem);

    nameMeson   = Form("%s_Meson2DTheta%s", plotPrefix.Data(), plotSuffix.Data());
    nameCanvas  = "Meson2DThetaCanvas";
    namePad     = "Meson2DThetaPad";
    cout << nameMeson.Data() << endl;
    Plot2DInPtBins( fHistoMappingGGInvMassThetaPtBin, "#theta", nameMeson, nameCanvas, namePad, fMesonMassPlotRange, fdate, fPrefix, fRow, fColumn, fStartPtBin, fNBinsPt,
                        fBinsPt, fTextMeasurement, fIsMC ,fDecayChannel, fDetectionProcess, fCollisionSystem);
    ...
    ///*********************** InvMass vs alpha
    TCanvas* canvasIA = new TCanvas("canvasIA","",1550,1200);  // gives the page size
    canvasIA->SetLogx(0);
    canvasIA->SetLogy(0);

    TH2D* temp = (TH2D*) fGammaGammaInvMassVSAlpha->Project3D("yx");
    if (fPrefix.CompareTo("Pi0") ==0 || fPrefix.CompareTo("Pi0EtaBinning")==0){
        DrawAutoGammaHisto2D( temp,
                                "", "m_{inv} (GeV/c^2)", Form("InvMass vs. Alpha"),"",
                                kFALSE, 3.,0.,
                                kFALSE, 0.132,0.140
                               );
    }
    canvasIA->Update();
    canvasIA->SaveAs(Form("%s/%s_InvMass_Alpha_%s.%s",outputDirMon.Data(),fPrefix.Data(),fCutSelection.Data(),Suffix.Data()));

    ///*********************** InvMass vs theta
    canvasIA->Clear();
    temp = (TH2D*) fGammaGammaInvMassVSTheta->Project3D("yx");
    if (fPrefix.CompareTo("Pi0") ==0 || fPrefix.CompareTo("Pi0EtaBinning")==0){
        DrawAutoGammaHisto2D( temp,
                                "", "m_{inv} (GeV/c^2)", Form("InvMass vs. Theta"),"",
                                kFALSE, 3.,0.,
                                kFALSE, 0.132,0.140
                                );
    }
    canvasIA->Update();
    canvasIA->SaveAs(Form("%s/%s_InvMass_Theta_%s.%s",outputDirMon.Data(),fPrefix.Data(),fCutSelection.Data(),Suffix.Data()));
    // **************************************************************************************************************
...
//                    fMesonIntDeltaRangeNarrow[1]    = fMesonIntDeltaRange[1]*0.4; // mod.
    ...
    fHistoMappingGGAlphaPtBin                                       = new TH1D*[fNBinsPt];
    fHistoMappingBackAlphaPtBin                                     = new TH1D*[fNBinsPt];
    fHistoMappingGGThetaPtBin                                       = new TH1D*[fNBinsPt];
    fHistoMappingGGInvMassAlphaPtBin                                = new TH2D*[fNBinsPt];
    fHistoMappingGGInvMassThetaPtBin                                = new TH2D*[fNBinsPt];
    fHistoMappingBackThetaPtBin                                     = new TH1D*[fNBinsPt];
    fHistoMappingBackNormInvMassPtBin                               = new TH1D*[fNBinsPt];
    fHistoMappingSignalInvMassPtBin                                 = new TH1D*[fNBinsPt];
    fHistoMappingBackNormAlphaPtBin                                 = new TH1D*[fNBinsPt];
    fHistoMappingSignalAlphaPtBin                                   = new TH1D*[fNBinsPt];
    fHistoMappingBackNormThetaPtBin                                 = new TH1D*[fNBinsPt];
    fHistoMappingSignalThetaPtBin                                   = new TH1D*[fNBinsPt];
    ...
    for(Int_t i = 0;i<fNBinsPt; i++){
        ...
        fHistoMappingGGAlphaPtBin[i]                                        = NULL;
        fHistoMappingBackAlphaPtBin[i]                                      = NULL;
        fHistoMappingGGThetaPtBin[i]                                        = NULL;
        fHistoMappingGGInvMassAlphaPtBin[i]                                 = NULL;
        fHistoMappingGGInvMassThetaPtBin[i]                                 = NULL;
        fHistoMappingBackThetaPtBin[i]                                      = NULL;
...
//****************************************************************************
//******************** Projection out of 3D in XY *****************************
//****************************************************************************
TH2D* FillProjectionYX (TH3* fDummy2D, TString name, Double_t minY, Double_t maxY){
    fDummy2D->GetZaxis()->SetRange(fDummy2D->GetZaxis()->FindBin(minY),fDummy2D->GetZaxis()->FindBin(maxY));
    TH2D* dummy2D = (TH2D*)fDummy2D->Project3D("yx");
    dummy2D->SetName(name.Data());
    return dummy2D;
}
...
void CheckForNULLForPointer(TH2D* fDummy1){
    if(fDummy1!= NULL){
        delete fDummy1;
        fDummy1           = NULL;
    }
...
//******* Fill array of invariant mass histograms in pT slices ***************
//****************************************************************************
void FillAlphaHistosArray(TH2D* fGammaGammaAlphaVSPtDummy) {
    fGammaGammaAlphaVSPtDummy->Sumw2();
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){
        fNameHistoGG    = Form("Mapping_GG_Alpha_in_Pt_Bin%02d", iPt);
        CheckForNULLForPointer(fHistoMappingGGAlphaPtBin[iPt]);
        fHistoMappingGGAlphaPtBin[iPt]=  FillProjectionX(fGammaGammaAlphaVSPtDummy, fNameHistoGG, fBinsPt[iPt], fBinsPt[iPt+1], 1);
    }
}
...
//******* Fill array of invariant mass histograms in pT slices ***************
//****************************************************************************
void FillAlphaBckHistosArray(TH2D* fGammaGammaAlphaVSPtDummy) {
    fGammaGammaAlphaVSPtDummy->Sumw2();
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){
        fNameHistoGG    = Form("Mapping_GG_Alpha_in_Pt_Bin%02d", iPt);
        CheckForNULLForPointer(fHistoMappingBackAlphaPtBin[iPt]);
        fHistoMappingBackAlphaPtBin[iPt]=  FillProjectionX(fGammaGammaAlphaVSPtDummy, fNameHistoGG, fBinsPt[iPt], fBinsPt[iPt+1], 1);
    }
}
...
//******* Fill array of invariant mass histograms in pT slices ***************
//****************************************************************************
void FillThetaHistosArray(TH2D* fGammaGammaThetaVSPtDummy) {
    fGammaGammaThetaVSPtDummy->Sumw2();
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){
        fNameHistoGG    = Form("Mapping_GG_Theta_in_Pt_Bin%02d", iPt);
        CheckForNULLForPointer(fHistoMappingGGThetaPtBin[iPt]);
        fHistoMappingGGThetaPtBin[iPt]=  FillProjectionX(fGammaGammaThetaVSPtDummy, fNameHistoGG, fBinsPt[iPt], fBinsPt[iPt+1], 1);
    }
}
...
//******* Fill array of invariant mass histograms in pT slices ***************
//****************************************************************************
void FillThetaBckHistosArray(TH2D* fGammaGammaThetaVSPtDummy) {
    fGammaGammaThetaVSPtDummy->Sumw2();
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){
        fNameHistoGG    = Form("Mapping_GG_Theta_in_Pt_Bin%02d", iPt);
        CheckForNULLForPointer(fHistoMappingBackThetaPtBin[iPt]);
        fHistoMappingBackThetaPtBin[iPt]=  FillProjectionX(fGammaGammaThetaVSPtDummy, fNameHistoGG, fBinsPt[iPt], fBinsPt[iPt+1], 1);
    }
}
...
//******* Fill array of invariant mass histograms in pT slices ***************
//****************************************************************************
void FillInvMassAlphaHistosArray(TH3D* fGammaGammaAlphaVSPtDummy) {
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){
        fNameHistoGG    = Form("Mapping_GG_InvMassAlpha_in_Pt_Bin%02d", iPt);
        CheckForNULLForPointer(fHistoMappingGGInvMassAlphaPtBin[iPt]);
        fHistoMappingGGInvMassAlphaPtBin[iPt]=  FillProjectionYX(fGammaGammaAlphaVSPtDummy, fNameHistoGG, fBinsPt[iPt], fBinsPt[iPt+1]);
    }
}
...
//******* Fill array of invariant mass histograms in pT slices ***************
//****************************************************************************
void FillInvMassThetaHistosArray(TH3D* fGammaGammaAlphaVSPtDummy) {
    for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){
        fNameHistoGG    = Form("Mapping_GG_InvMassTheta_in_Pt_Bin%02d", iPt);
        CheckForNULLForPointer(fHistoMappingGGInvMassThetaPtBin[iPt]);
        fHistoMappingGGInvMassThetaPtBin[iPt]=  FillProjectionYX(fGammaGammaAlphaVSPtDummy, fNameHistoGG, fBinsPt[iPt], fBinsPt[iPt+1]);
    }
}
...
//****************************************************************************
void FitSubtractedPol2InvMassInPtBins(TH1D* fHistoMappingSignalInvMassPtBinSingle, Double_t* fMesonIntDeltaRangeFit, Int_t ptBin, Bool_t vary){
...
//****************************************************************************
void FitSubtractedExp1InvMassInPtBins(TH1D* fHistoMappingSignalInvMassPtBinSingle, Double_t* fMesonIntDeltaRangeFit, Int_t ptBin, Bool_t vary){
    ...
    fFitLinearBck   = NULL;
    fFitLinearBck   = new TF1("BGfit","[0]*TMath::Exp([1]*x)",fMesonFitRange[0],fMesonFitRange[1]);
...
//****************************************************************************
void FitSubtractedExp2InvMassInPtBins(TH1D* fHistoMappingSignalInvMassPtBinSingle, Double_t* fMesonIntDeltaRangeFit, Int_t ptBin, Bool_t vary){
...
void SaveHistos(Int_t optionMC, TString fCutID, TString fPrefix3, Bool_t UseTHnSparse ) {
    TString nameOutput = Form("theta_%.4f/%s/%s/%s_%s_GammaConvV1WithoutCorrection%s_%s.root",fCutTheta,fCutSelection.Data(),fEnergyFlag.Data(),fPrefix.Data(),fPrefix3.Data(),fPeriodFlag.Data(),fCutID.Data());
    ...
    for(Int_t ii =fStartPtBin;ii<fNBinsPt;ii++){
        ...
        fHistoMappingGGAlphaPtBin[ii]->Write();
        fHistoMappingGGThetaPtBin[ii]->Write();
...
void SaveCorrectionHistos(TString fCutID, TString fPrefix3){
    const char* nameOutput = Form("theta_%.4f/%s/%s/%s_%s_GammaConvV1CorrectionHistos%s_%s.root",fCutTheta,fCutSelection.Data(),fEnergyFlag.Data(),fPrefix.Data(),fPrefix3.Data(),fPeriodFlag.Data(),fCutID.Data());
...
void Delete(){
    ...
    if (fHistoMappingBackAlphaPtBin)                            delete fHistoMappingBackAlphaPtBin;
    if (fHistoMappingBackThetaPtBin)                            delete fHistoMappingBackThetaPtBin;
    ...
    if (fHistoMappingSignalAlphaPtBin)                          delete fHistoMappingSignalAlphaPtBin;
    if (fHistoMappingSignalThetaPtBin)                          delete fHistoMappingSignalThetaPtBin;
