//**********************************************************************************************
//**************************** CutStudiesOverviewOmega *****************************************
//**********************************************************************************************
/***********************************************************************************************
 ******     provided by Gamma Conversion Group, PWG4,                                    *******
 ******        Ana Marin, marin@physi.uni-heidelberg.de                                  *******
 ******        Friederike Bock, friederike.bock@cern.ch                                  *******
 ****** adapted for omega by Florian Jonas, florian.jonas@cern.ch                        *******
 ***********************************************************************************************/
...
void CutStudiesOverviewOmega(TString CombineCutsName                 = "CombineCuts.dat",
    ...
    TString process             = Form("%s #rightarrow #pi^{+}#pi^{-}#pi^{0}", textMeson.Data());
    ...
    for (Int_t i=0; i< NumberOfCuts; i++){
        ...
        if (correctionFilesAvail){
            ...
            if(i == 0){
                ...
                cout << "fEventCutSelection = " << fEventCutSelection.Data() << endl;
                cout << "centralityString = " << centralityString.Data() << endl;
            ...
            // Ratios are not there for omega
            /*
        ...
        histoRawYieldCut[i]                                     = (TH1D*)Cutuncorrfile[i]->Get(Form("histoYieldMesonPerEvent%s",InvMassTypeEnding.Data()));
        ...
        histoWidthMeson[i]                                      = (TH1D*)Cutuncorrfile[i]->Get(Form("histoFWHMMeson%s",InvMassTypeEnding.Data()));
        histoWidthMeson[i]->SetName(Form("histoFWHMMeson_%s",cutNumber[i].Data()));

        histoSBCut[i]                                           = (TH1D*)Cutuncorrfile[i]->Get(Form("histoSBdefaultMeson%s",InvMassTypeEnding.Data()));
        histoSBCut[i]->SetName(Form("histoSBdefaultMeson_%s",cutNumber[i].Data()));

        histoSBNarrowCut[i]                                     = (TH1D*)Cutuncorrfile[i]->Get(Form("histoSBdefaultNarrowMeson%s",InvMassTypeEnding.Data()));
        histoSBNarrowCut[i]->SetName(Form("histoSBdefaultNarrowMeson_%s",cutNumber[i].Data()));

        histoSignCut[i]                                         = (TH1D*)Cutuncorrfile[i]->Get(Form("histoSigndefaultMeson%s",InvMassTypeEnding.Data()));
        histoSignCut[i]->SetName(Form("histoSigndefaultMeson_%s",cutNumber[i].Data()));

        histoSignNarrowCut[i]                                   = (TH1D*)Cutuncorrfile[i]->Get(Form("histoSigndefaultNarrowMeson%s",InvMassTypeEnding.Data()));
        histoSignNarrowCut[i]->SetName(Form("histoSigndefaultNarrowMeson_%s",cutNumber[i].Data()));

        // removed ClusterPtPerEvent because it is not there for omega
    ...
    // Removed everything with ClusterPt because it is not included yet in ExtractSignal
    ...
    if (correctionFilesAvail){
      ...
      histoRatioCorrectedYieldTrueNarrow->DrawCopy("same,e1,p");
        ...
        //************************************* Not plotting Ratios for now, because they are not implemented in omega Correction Afterburner yet !
        ...
        //*************************************************************************************
        //***********************Optional pure photon part not implemented for now*************
        //*************************************************************************************
