...
// Main Function
void AnalyseDCADist(    TString meson           ="",
                        TString fileData        ="",
                        TString fileData2       ="" ,
    ...
    // if only one filename is given, dca tree and ESD histograms have to be in this one file
    Bool_t allInOne = kFALSE;
    if(fileData2.CompareTo("")==0) allInOne=kTRUE;

    // possibility to process subfiles
    ifstream fileListName;
    TString tempLine;
    TString fileNamesInList[100];
    TString fileNamesInList2[100];
    Int_t nFilesInList = 0;
    Int_t nFilesInList2 = 0;
    if(fileData.Contains(".txt")){  // given txt file containing root file names
      fileListName.open(fileData,ios_base::in);
      if (!fileListName) {
	cout << "ERROR: " << fileData.Data() << " not found!" << endl;
	return;
      } // go through list of root files and check if they can be opened
      for( TString tempLine; tempLine.ReadLine(fileListName, kTRUE); ) {
	cout << tempLine.Data() << endl;
	TFile* f = new TFile(tempLine.Data());
	if(!f->IsZombie()) {
	...
	delete f;
      ...
      fileListName.close();
      if(!allInOne){ //repeat the same for the other text file -> fileNamesInList2
	fileListName.open(fileData2,ios_base::in);
	if (!fileListName) {
	  cout << "ERROR: " << fileData2.Data() << " not found!" << endl;
	  return;
	} // go through list of root files and check if they can be opened
	for( TString tempLine; tempLine.ReadLine(fileListName, kTRUE); ) {
	  cout << tempLine.Data() << endl;
	  TFile* f = new TFile(tempLine.Data());
	  if(!f->IsZombie()) {
	    fileNamesInList2[nFilesInList2] = tempLine;
	    nFilesInList2++;
	  }
	  f->Close();
	  delete f;
	...
	fileListName.close();
	if (nFilesInList2 != nFilesInList){
	  cout << "ERROR: different number of files in file 1 and file 2" << endl;
	  return;
	}
      }
    }
      ...
      nFilesInList=1;
      fileNamesInList[0] = fileData;
      if(!allInOne) fileNamesInList2[0] = fileData2;
    }
    ...
    //##########################################
    //####### START LOOP OVER (SUB)FILES #######
    //##########################################

    cout << "Will process " << nFilesInList << " file(s)... " << endl;

    for (Int_t readFile = 0; readFile < nFilesInList; readFile++ ){

      // open file and get DCA container:
      TString fileName                        = fileNamesInList[readFile];
	...
	ESDContainer                          = (TList*) HistosGammaConversion->FindObject(Form("%s ESD histograms", fCutSelection.Data()));
      } else {  // open second file if neccessary to get ESD container:
	TString fileName2                        = fileNamesInList2[readFile];
	...
	cout << "NEvents in this file: " << fEventQuality->GetBinContent(1) << endl;
      }
      else {
	TH1D* fEventQualityTmp         = (TH1D*)ESDContainer->FindObject("NEvents");
	cout << "NEvents in this file: " << fEventQualityTmp->GetBinContent(1) << endl;
	fEventQuality->Add(fEventQualityTmp);
      }
      ...
      if (readFile == 0){  // create new histograms
	for (Int_t i = 0; i < 6 ; i++){
	  for (Int_t j = fStartPtBin; j < fNBinsPt ; j++){
	...
	fHistDCAZUnderMesonAllCat_AllPt                       = new TH1F("HistDCAZUnderMesonAllCat_AllPt", "HistDCAZUnderMesonAllCat_AllPt",
									       201, -10, 10);
	fHistDCAZUnderMesonAllCat_AllPt->Sumw2();
	fHistDCAZTruePrimaryMesonGammaGammaAllCat_AllPt       = new TH1F("HistDCAZTruePrimaryMesonGammaGammaAllCat_AllPt", "HistDCAZTruePrimaryMesonGammaGammaAllCat_AllPt",
									       201, -10, 10);
	fHistDCAZTruePrimaryMesonGammaGammaAllCat_AllPt->Sumw2();
	fHistDCAZTruePrimaryMesonDalitzAllCat_AllPt           = new TH1F("HistDCAZTruePrimaryMesonDalitzAllCat_AllPt", "HistDCAZTruePrimaryMesonDalitzAllCat_AllPt",
									       201, -10, 10);
	fHistDCAZTruePrimaryMesonDalitzAllCat_AllPt->Sumw2();
	fHistDCAZTrueSecondaryMesonFromK0sAllCat_AllPt        = new TH1F("HistDCAZTrueSecondaryMesonFromK0sAllCat_AllPt", "HistDCAZTrueSecondaryMesonFromK0sAllCat_AllPt",
									       201, -10, 10);
	fHistDCAZTrueSecondaryMesonFromK0sAllCat_AllPt->Sumw2();
	fHistDCAZTrueSecondaryMesonFromEtaAllCat_AllPt        = new TH1F("HistDCAZTrueSecondaryMesonFromEtaAllCat_AllPt", "HistDCAZTrueSecondaryMesonFromEtaAllCat_AllPt",
									       201, -10, 10);
	fHistDCAZTrueSecondaryMesonFromEtaAllCat_AllPt->Sumw2();
	fHistDCAZTrueSecondaryMesonFromSomethingAllCat_AllPt  = new TH1F("HistDCAZTrueSecondaryMesonFromSomethingAllCat_AllPt", "HistDCAZTrueSecondaryMesonFromSomethingAllCat_AllPt",
									       201, -10, 10);
	fHistDCAZTrueSecondaryMesonFromSomethingAllCat_AllPt->Sumw2();
	fHistDCAZTrueBackgroundAllCat_AllPt                   = new TH1F("HistDCAZTrueBackgroundAllCat_AllPt", "HistDCAZTrueBackgroundAllCat_AllPt",
									       201, -10, 10);
	fHistDCAZTrueBackgroundAllCat_AllPt->Sumw2();
	fHistDCAZGarbageAllCat_AllPt                          = new TH1F("HistDCAZGarbageAllCat_AllPt", "HistDCAZGarbageAllCat_AllPt",
									       201, -10, 10);
	...
	for (Int_t i = 0; i < 6 ; i++){
      ...
      } // end of creating new histograms in case of first file
      ...
      cout << "Process tree with " << nentries << " entries..." << endl;
    ...
    Double_t intBGHist                  [5][6][fNBinsPt];
    Double_t intErrBGHist               [5][6][fNBinsPt];
    Double_t fractionIntHist            [5][6][fNBinsPt];
    Double_t fractionErrIntHist         [5][6][fNBinsPt];
    ...
    for (Int_t i = 0; i < 6 ; i++){
        ...
        for (Int_t j = fStartPtBin; j < fNBinsPt ; j++){
            ...
            for (Int_t k = 0; k < 5; k ++){
                if ( k == 0){
                    if (i > 0) fHistDCAZUnderMesonBGEstimate_MesonPt[k][i][j]  = (TH1F*)fHistDCAZUnderMeson_MesonPt[i][j]->ShowBackground(nIterBGFit+2,optionBGSmoothingStandard.Data());
                    ...
                    if (i > 0) fHistDCAZUnderMesonBGEstimate_MesonPt[k][i][j]  = (TH1F*)fHistDCAZUnderMeson_MesonPt[i][j]->ShowBackground(nIterBGFit+2,optionBGSmoothingVar2.Data());
                    ...
                    if (i > 0) fHistDCAZUnderMesonBGEstimate_MesonPt[k][i][j]  = (TH1F*)fHistDCAZUnderMeson_MesonPt[i][j]->ShowBackground(nIterBGFit+2,optionBGSmoothingVar1.Data());
                    ...
                    if (i > 0) fHistDCAZUnderMesonBGEstimate_MesonPt[k][i][j]  = (TH1F*)fHistDCAZUnderMeson_MesonPt[i][j]->ShowBackground(nIterBGFit+1+2,optionBGSmoothingStandard.Data());
                    else fHistDCAZUnderMesonBGEstimate_MesonPt[k][i][j]         = (TH1F*)fHistDCAZUnderMeson_MesonPt[i][j]->ShowBackground(nIterBGFit+1,optionBGSmoothingStandard.Data());
                } else if (k == 4){
                    if (i > 0) fHistDCAZUnderMesonBGEstimate_MesonPt[k][i][j]  = (TH1F*)fHistDCAZUnderMeson_MesonPt[i][j]->ShowBackground(nIterBGFit-1+2,optionBGSmoothingStandard.Data());
                    else fHistDCAZUnderMesonBGEstimate_MesonPt[k][i][j]         = (TH1F*)fHistDCAZUnderMeson_MesonPt[i][j]->ShowBackground(nIterBGFit-1,optionBGSmoothingStandard.Data());
                }
    ...
    TH1D* fHistFracIntHistBGvsPt                [5][6] ;
    TH1D* fHistFracIntHistBGFittedvsPt          [5][6] ;
    TH1D* fHistFracCatvsPt                      [6] ;
    TH1D* fHistCorrectionFactorsHistCat         [5][6];
    ...
    Style_t  styleCat[6]        = { kFullCircle, kFullSquare, kFullStar, kOpenDiamond, kOpenCircle, kOpenCross};

    TF1* fitFracIntHistBGvsPt[5][6];
    ...
    canvasCorrFrac->cd();
        ...
        TLegend* legendFracIntHistBG    = GetAndSetLegend2(0.75, 0.95-3*0.04, 0.95, 0.95, 0.04, 1, "", 42, 0.2);
        ...
        for (Int_t k = 0; k< 5; k++){
            for (Int_t i = 0; i< nCatFit; i++){
	         if (i == 0){
                ...
                fitFracIntHistBGvsPt[k][i]                 = new TF1(Form("fitFracIntHistBGvsPt_%i_%i",k,i),"[0]/TMath::Power(x,[1])");
		    ...
		    Double_t bgEstimate                 = (fitFracIntHistBGvsPt[k][i]->Integral(ptStart, ptEnd) / binWidth );
                ...
                fitFracIntHistBGvsPt[k][i]->Draw("same");
                if (k == 0)legendFracIntHistBG->AddEntry(fHistFracIntHistBGvsPt[k][i],Form("Category %i",i+1),"p");
            }
            legendFracIntHistBG->Draw();
            labelEnergy->Draw();
            labelMeson->Draw();
            canvasCorrFrac->Update();
            canvasCorrFrac->SaveAs(Form("%s/%s_%s_FracBGOverIntHist_Method%d.%s", outputDir.Data(), meson.Data(), fMCFlag.Data(), k, suffix.Data()));
        }
    ...
    Color_t colorMethod[5]                  = {kRed+2, kGreen+2, kBlue+2, 807, kViolet+1};
    Style_t styleMethod[5]                  = {kFullSquare, kFullDiamond, kFullStar, kOpenDiamond, kOpenCross};
    Size_t sizeMethod[5]                    = {2., 3., 3.5, 3., 2.};
    TString nameMethod[5]                   = {"A", "C", "D", "A+ite","A-ite" };
    Int_t nMethods                          = 6;
    ...
    canvasCorrFrac->cd();
        DrawAutoGammaMesonHistos( fHistCorrectionFactorsHistAllCat,
                                ...
                                kFALSE, 0., 10.);
        DrawGammaSetMarker(fHistCorrectionFactorsHistAllCat, 20, 2*0.8, kBlack, kBlack);
        fHistCorrectionFactorsHistAllCat->GetYaxis()->SetTitleOffset(0.7);
        fHistCorrectionFactorsHistAllCat->DrawCopy("e1,p");

        if (doFitEstimate){
            DrawGammaSetMarker(fHistCorrectionFactorsFitAllCat, 25, 2*0.8, kCyan+2, kCyan+2);
            fHistCorrectionFactorsFitAllCat->DrawCopy("same,e1,p");
            nMethods++;
        }

        TLegend* legendCorrFraction         = GetAndSetLegend2(0.5, 0.95-nMethods*0.04, 0.95, 0.95, 0.04, 1, "", 42, 0.15);
        ...
        for (Int_t k = 0; k < 5 ; k++){
            DrawGammaSetMarker(fHistCorrectionFactorsHist[k], styleMethod[k], sizeMethod[k]*0.8, colorMethod[k], colorMethod[k]);
    ...
    canvasCorrFrac->cd();
        DrawAutoGammaMesonHistos( fHistFracCatvsPt[0],
                                "", "#it{p}_{T,meson} (GeV/#it{c})", "#it{N}_{meson per cat}/(#it{N}_{meson}) (%)",
                                kFALSE, 2.,1e-8, kFALSE,
                                kTRUE, 0, 100,
                                kFALSE, 0., 10.);
        DrawGammaSetMarker(fHistFracCatvsPt[0], styleCat[0], 1., colorCat[0], colorCat[0]);
        fHistFracCatvsPt[0]->GetYaxis()->SetTitleOffset(0.7);
    ...
    for (Int_t j = fStartPtBin; j < fNBinsPt; j++){
        ...
        fHistDCAZUnderMesonBG2_Visual_AllCat->Scale(1./fHistDCAZUnderMesonBG2_Visual_AllCat->GetEntries());
            ...
            TLatex *labelPtBin = new TLatex(0.63,0.9,Form("%1.2f < #it{p}_{T} (GeV/#it{c}) < %1.2f", fBinsPt[j], fBinsPt[j+1]));
    ...
    for (Int_t j = fStartPtBin; j < fNBinsPt; j++){
        ...
        SetStyleHistoTH2ForGraphs( histoDCACatCompareDummy, "dca_{#it{z}} (cm)", "normalized counts",
        ...
        for (Int_t i = 0; i < 6 ; i++){
            if(fHistDCAZUnderMeson_MesonPt[i][j]){
                ...
                DrawGammaSetMarker(fHistDCAZUnderMeson_Visual_Cat, styleCat[i], markersizePlot[i], colorCat[i] , colorCat[i]);
                fHistDCAZUnderMeson_Visual_Cat->Draw("p,same,e");
                legendDCACatCompare->AddEntry(fHistDCAZUnderMeson_Visual_Cat,Form("category %d",i+1),"p");
            }
        }
...
void PlotDCADistPtBinWithFitAndEstimateCat(TString namePlot, TString nameCanvas, TString namePad,
    ...
    for(Int_t iPt = fStartBinPtRange; iPt < fNumberPtBins; iPt++){
        ...
        if (place == fColumnPlot){
            ...
            if (fHistDCAZUnderMesonBGEstimate_MesonPt[3][category-1][fStartBinPtRange])
                legend->AddEntry(fHistDCAZUnderMesonBGEstimate_MesonPt[3][category-1][fStartBinPtRange],"Pileup Estimate Method A + 1 ite.","l");
            if (fHistDCAZUnderMesonBGEstimate_MesonPt[4][category-1][fStartBinPtRange])
                legend->AddEntry(fHistDCAZUnderMesonBGEstimate_MesonPt[4][category-1][fStartBinPtRange],"Pileup Estimate Method A - 1 ite.","l");
            legend->Draw();
            ...
            if (fHistDCAZUnderMesonBGEstimate_MesonPt[3][category-1][iPt]){
                DrawGammaDCAHisto( fHistDCAZUnderMesonBGEstimate_MesonPt[3][category-1][iPt],
                                Form("%3.2f GeV/c < #it{p}_{T,%s} < %3.2f GeV/c", startPt, fMesonType.Data(), endPt),
                                "dca_{z} #gamma (cm)", Form("d(dca_{z}), cat %i", category),
                                -fMaxDcaZPhoton, fMaxDcaZPhoton, 4, 0.5e4);
            }
            if (fHistDCAZUnderMesonBGEstimate_MesonPt[4][category-1][iPt]){
                DrawGammaDCAHisto( fHistDCAZUnderMesonBGEstimate_MesonPt[4][category-1][iPt],
                                Form("%3.2f GeV/c < #it{p}_{T,%s} < %3.2f GeV/c", startPt, fMesonType.Data(), endPt),
                                "dca_{z} #gamma (cm)", Form("d(dca_{z}), cat %i", category),
